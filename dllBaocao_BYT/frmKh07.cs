...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh07 : System.Windows.Forms.Form
	{
		...
		public frmKh07(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh07( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh07));
            ...
            this.butMoi.Location = new System.Drawing.Point(95, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(96, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(191, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(96, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(287, 500);
            this.butLuu.Name = "butLuu";
            this.butLuu.Size = new System.Drawing.Size(96, 25);
            ...
            this.butBoqua.Location = new System.Drawing.Point(383, 500);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(96, 25);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(575, 500);
            ...
            this.butHuy.Size = new System.Drawing.Size(96, 25);
            ...
            this.Text = "Hoạt động khám chữa phụ khoa và nạo phá thai";
		...
		private void frmKh07_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_07_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_07 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_07 a,"+m.user+".khdm_07 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_07 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_07 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_07 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_07 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh07_moi(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c08"].ToString()),int.Parse(r[i]["c09"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                if (i_col >= 3 && i_col <= 11 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        {
            decimal l_tong = decimal.Parse(dataGrid1[row, 6].ToString()) + decimal.Parse(dataGrid1[row, 7].ToString()) + decimal.Parse(dataGrid1[row, 8].ToString());
            ...
            if (decimal.Parse(dataGrid1[row, 4].ToString()) < 0)
            {
            ...
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;

            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || decimal.Parse(dataGrid1[row, 9].ToString()) > l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            ...
            if (decimal.Parse(dataGrid1[row, 11].ToString()) < 0 || decimal.Parse(dataGrid1[row, 11].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }

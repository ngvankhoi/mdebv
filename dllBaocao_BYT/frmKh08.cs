...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh08 : System.Windows.Forms.Form
	{
		...
		private int i_userid,i_mabv,i_tong=-1,i_col,i_row=0;
		...
		public frmKh08(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh08( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh08));
            ...
            this.butMoi.Location = new System.Drawing.Point(146, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(228, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(310, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(474, 500);
            ...
            this.Text = "Hoạt động cung cấp dịch vụ kế hoạch hóa gia đình";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh08_Load);
		...
		private void frmKh08_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_08_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + m.user + ".khdm_08 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten from " + m.user + ".khbieu_08 a," + m.user + ".khdm_08 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + m.user + ".khdm_08 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_08 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_08 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_08 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh08(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c09"].ToString()),int.Parse(r[i]["c10"].ToString()), i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                string exp = "ma>=51 and ma<150", stt = "II";
                if ((i_ma >= 1 && i_ma <= 50) || (dataGrid1[i_row, 1].ToString() == "I"))
                {
                    exp = "ma>=1 and ma<=49";
                    stt = "I";
                }
                else
                    if ((i_ma >= 51 && i_ma <= 150) || (dataGrid1[i_row, 1].ToString() == "II"))
                    {
                        exp = "ma>=51 and ma <=149";
                        stt = "II";
                    }
                    else
                        if (i_ma >= 151)
                ...
                m.updrec_050(ds.Tables[0], stt, i_col, l_tong);
                if (i_col >= 3 && i_col <= 12 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        {
            decimal l_tong = decimal.Parse(dataGrid1[row, 4].ToString()) + decimal.Parse(dataGrid1[row, 5].ToString()) + decimal.Parse(dataGrid1[row, 6].ToString()) + decimal.Parse(dataGrid1[row, 7].ToString()) + decimal.Parse(dataGrid1[row, 8].ToString());
            ...
            if (decimal.Parse(dataGrid1[row, 4].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
            ...
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0 || decimal.Parse(dataGrid1[row, 5].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 6].ToString()) < 0 || decimal.Parse(dataGrid1[row, 6].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0 || decimal.Parse(dataGrid1[row, 7].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || decimal.Parse(dataGrid1[row, 9].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < l_tong)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            if (decimal.Parse(dataGrid1[row, 11].ToString()) < 0 || decimal.Parse(dataGrid1[row, 11].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
        ...
        private void Tongcong()
        {
            ...
            {
                sum("ma>=1 and ma<=49", "I", k);
                sum("ma>=51 and ma<=149", "II", k);
                sum("ma>=151 ", "B", k);
            }
        }
        ...
        private void sum(string exp, string stt, int k)
        {
            ...
            m.updrec_050(ds.Tables[0], stt, k, l_tong);
        }

...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh04 : System.Windows.Forms.Form
	{
		...
		public frmKh04(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh04( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh04));
            ...
            this.butMoi.Location = new System.Drawing.Point(155, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(77, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(232, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(77, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(309, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(386, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(540, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(463, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(77, 25);
            ...
            this.Text = "Tình hình thu chi ngân sách của tuyến xã";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh04_Load);
		...
		private void frmKh04_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_04_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_04 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_04 a,"+m.user+".khdm_04 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_04 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_04 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_04 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_04 where id="+l_id);
					...
					{
						m.upd_kh04(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c17"].ToString()),int.Parse(r[i]["c18"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                //i_row=dataGrid1.CurrentCell.RowNumber;
                if (i_col < 3) return;
                string exp = "ma>0 and ma<> 15", stt = "A";
                DataRow[] r = ds.Tables[0].Select(exp);
                Int16 iRec = Convert.ToInt16(r.Length);
        ...
        bool kiemtranhap(int col, int row)
        {
            decimal l_cot = decimal.Parse(dataGrid1[row, 5].ToString()) + decimal.Parse(dataGrid1[row, 7].ToString()) + decimal.Parse(dataGrid1[row, 9].ToString()) +
                decimal.Parse(dataGrid1[row, 11].ToString()) + decimal.Parse(dataGrid1[row, 13].ToString()) + decimal.Parse(dataGrid1[row, 15].ToString()) +
                decimal.Parse(dataGrid1[row, 17].ToString()) + decimal.Parse(dataGrid1[row, 19].ToString());
            if (decimal.Parse(dataGrid1[14, col].ToString()) < decimal.Parse(dataGrid1[15, col].ToString()))
            {
            ...
            if (decimal.Parse(dataGrid1[row, 4].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 6].ToString()) < 0 || decimal.Parse(dataGrid1[row, 6].ToString()) > decimal.Parse(dataGrid1[row, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) > decimal.Parse(dataGrid1[row, 7].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 10].ToString()) < 0 || decimal.Parse(dataGrid1[row, 10].ToString()) > decimal.Parse(dataGrid1[row, 9].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 11].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 12].ToString()) < 0 || decimal.Parse(dataGrid1[row, 12].ToString()) > decimal.Parse(dataGrid1[row, 11].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;

            }
            if (decimal.Parse(dataGrid1[row, 13].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 14].ToString()) < 0 || decimal.Parse(dataGrid1[row, 14].ToString()) > decimal.Parse(dataGrid1[row, 13].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 15].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 16].ToString()) < 0 || decimal.Parse(dataGrid1[row, 16].ToString()) > decimal.Parse(dataGrid1[row, 15].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 17].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 18].ToString()) < 0 || decimal.Parse(dataGrid1[row, 18].ToString()) > decimal.Parse(dataGrid1[row, 17].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 19].ToString()) < 0 || l_cot > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }
            if (decimal.Parse(dataGrid1[row, 20].ToString()) < 0 || decimal.Parse(dataGrid1[row, 20].ToString()) > decimal.Parse(dataGrid1[row, 19].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return false;
            }

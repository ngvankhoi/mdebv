...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh11 : System.Windows.Forms.Form
	{
		...
		public frmKh11(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh11( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh11));
            ...
            this.butMoi.Location = new System.Drawing.Point(162, 536);
            ...
            this.butSua.Location = new System.Drawing.Point(241, 536);
            ...
            this.butLuu.Location = new System.Drawing.Point(320, 536);
            ...
            this.butBoqua.Location = new System.Drawing.Point(399, 536);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(557, 536);
            ...
            this.butHuy.Location = new System.Drawing.Point(478, 536);
            ...
            this.Text = "Mắc chết các bệnh có Vacxin phòng ngừa";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh11_Load);
		...
		private void frmKh11_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_11_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_11 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_11 a,"+m.user+".khdm_11 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_11 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_11 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_11 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_11 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17+c18+c19+c20+c21+c22+c23+c24>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh11(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c23"].ToString()),int.Parse(r[i]["c24"].ToString()), i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Cơ sở y tế";
			TextCol1.Width = 190;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                if (i_col >= 3 && i_col <= 26 && dataGrid1[i_row, i_col].ToString() != "")
        ...
        #region kiem tra
        void kiemtranhap(int col, int row)
        {
            //decimal l_row = decimal.Parse(dataGrid1[row,5].ToString())+decimal.Parse(dataGrid1[row,6].ToString())+decimal.Parse(dataGrid1[row,7].ToString())+decimal.Parse(dataGrid1[row,8].ToString())+decimal.Parse(dataGrid1[row,9].ToString());
            ...
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            ...
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            ...
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            ...
            if (decimal.Parse(dataGrid1[row, 11].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            ...
            if (decimal.Parse(dataGrid1[row, 13].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            ...
            if (decimal.Parse(dataGrid1[row, 15].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            ...
            if (decimal.Parse(dataGrid1[row, 17].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            ...
            if (decimal.Parse(dataGrid1[row, 19].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            ...
            if (decimal.Parse(dataGrid1[row, 21].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row, 22].ToString()) < 0 || decimal.Parse(dataGrid1[row, 22].ToString()) > decimal.Parse(dataGrid1[row, 21].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row, 23].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row, 24].ToString()) < 0 || decimal.Parse(dataGrid1[row, 24].ToString()) > decimal.Parse(dataGrid1[row, 23].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row, 25].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row, 26].ToString()) < 0 || decimal.Parse(dataGrid1[row, 26].ToString()) > decimal.Parse(dataGrid1[row, 25].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            }

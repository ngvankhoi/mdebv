...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh123 : System.Windows.Forms.Form
	{
		...
		public frmKh123(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh123( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh123));
            ...
            this.butMoi.Location = new System.Drawing.Point(173, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(249, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(401, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(553, 500);
            ...
            this.Text = "Biểu 12.3//Công tác khám chữa bệnh và dịch vụ y tế";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh123_Load);
		...
		private void frmKh123_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_123_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_123 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_123 a,"+m.user+".kh_dm_123 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_123 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_123 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_123 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_123 where id="+l_id);
					...
					{
						m.upd_kh123(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),

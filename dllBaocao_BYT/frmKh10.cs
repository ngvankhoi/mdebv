...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh10 : System.Windows.Forms.Form
	{
		...
		public frmKh10(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh10( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh10));
            ...
            this.butMoi.Location = new System.Drawing.Point(133, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(85, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(218, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(85, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(303, 500);
            this.butLuu.Name = "butLuu";
            this.butLuu.Size = new System.Drawing.Size(85, 25);
            ...
            this.butBoqua.Location = new System.Drawing.Point(388, 500);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(85, 25);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(558, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(473, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(85, 25);
            ...
            this.Load += new System.EventHandler(this.frmKh10_Load);
		...
		private void frmKh10_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_10_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_10 order by to_number(stt)");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_10 a,"+m.user+".khdm_10 b where a.ma=b.ma and a.id="+l_id+" order by to_number(b.stt)";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_10 order by to_number(stt)");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_10 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_10 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			//	if (ds.HasChanges()) Tongcong();
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_10 where id="+l_id);
					...
					{
						m.upd_kh10(l_id,(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c11"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Tên Xã/Phường";
			TextCol1.Width = 190;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
        ...
        void kiemtranhap(int col, int row)
        {
            ...
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) < l_row)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 6].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) < l_row)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) < l_row)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) < l_row)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) < l_row)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);

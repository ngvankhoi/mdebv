...
namespace dllBaocao_BYT
{
	...
	public class frmKh06 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butMoi.Location = new System.Drawing.Point(164, 517);
            ...
            this.butSua.Location = new System.Drawing.Point(240, 517);
            ...
            this.butLuu.Location = new System.Drawing.Point(316, 517);
            ...
            this.butBoqua.Location = new System.Drawing.Point(392, 517);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(544, 517);
            ...
            this.butHuy.Location = new System.Drawing.Point(468, 517);
            ...
            this.Text = "Tình hình mắc chết do tai biến sản khoa";
		...
		private void frmKh06_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_06_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_06 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_06 a,"+m.user+".khdm_06 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_06 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_06 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_06 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
			DataTable dt1=m.get_data("select * from "+m.user+".khbieu_06 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_06 where id="+l_id);
					...
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh06(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c08"].ToString()),int.Parse(r[i]["c09"].ToString()),int.Parse(r[i]["c10"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;

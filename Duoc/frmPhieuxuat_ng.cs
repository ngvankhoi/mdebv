...
namespace Duoc
{
	...
	/// </summary>
	public class frmPhieuxuat_ng : System.Windows.Forms.Form
	{
		...
		private string sql,s_kho,s_makho,s_makp,s_loaint,s_tenkho,s_tenkp,s_tenloai,stime,user,xxx;
		...
		public frmPhieuxuat_ng(AccessData acc,int nhom,string kho,string kp,string loaint, int _userid)
		{
			//
		...
		private void get_xuat(string s_mmyy)
		{
            ...
            sql += " union all ";
            sql += "select b.mabd,round(b.giamua," + i_dongiale + ") as dongia,";
            sql += "sum(-1*b.soluong) as soluong from " + xxx + ".d_nhapll a," + xxx + ".d_nhapct b," + user + ".d_dmbd c ";
            ...
            if (s_makp != "") sql += " and a.chietkhau in (" + s_makp.Substring(0, s_makp.Length - 1) + ")";
            if (s_loaint != "") sql += " and a.lydo in (" + s_loaint.Substring(0, s_loaint.Length - 1) + ")";
            sql += " group by b.mabd,round(b.giamua," + i_dongiale + ")";
			ins_items();
		}
		...
		private void butIn_Click(object sender, System.EventArgs e)
		{
			...
			frmReport f=new frmReport(d,dsxml.Tables[0], i_userid,"d_phieuxuat.rpt","",tu.Text,"","",s_tenloai,"Xuất sử dụng",s_tenkho,doiso.Doiso_Unicode(Convert.ToInt64(d_tongcong).ToString()),"","");
			f.ShowDialog(this);
		}

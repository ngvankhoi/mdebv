...
namespace Duoc
{
	...
	public class frmDmbdtd : System.Windows.Forms.Form
	{
		...
		public frmDmbdtd(AccessData acc,int nhom,decimal id,string title,int userid)
		{
			//
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDmbdtd));
            ...
            this.butXoa.Location = new System.Drawing.Point(296, 380);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(366, 380);
            ...
            this.butLuu.Location = new System.Drawing.Point(226, 380);
		...
		{
            user = d.user; itable = d.tableid("", "d_dmbdtd");
		...
		{
			dsdm.Clear();
            dsdm.Merge(d.get_data("select a.id,a.ma,trim(a.ten)||' '||a.hamluong as ten,a.dang,a.tenhc,b.ten as tenhang from " + user + ".d_dmbd a," + user + ".d_dmhang b where a.mahang=b.id and a.id<>" + i_id + " and a.nhom=" + i_nhom + " and a.id not in (select mabd from " + user + ".d_dmbdtd where id=" + i_id + ") order by a.ten"));
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			if (dsdm.Tables[0].Select("chon=true").Length>0)
			{
				...
				foreach(DataRow r in dsdm.Tables[0].Select("chon=true")) d.upd_dmbdtd(i_id,int.Parse(r["id"].ToString()),1);
				foreach (DataRow row in dsdm.Tables[0].Rows) row["chon"]=false;
				load_chitiet();
			}
			else foreach(DataRow r in dt.Rows) d.upd_dmbdtd(i_id,int.Parse(r["mabd"].ToString()),decimal.Parse(r["soluong"].ToString()));
		}
		...
		private void butXoa_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                d.upd_eve_upd_del(itable, i_userid, "del", d.fields(user + ".d_dmbdtd", "id=" + i_id));
                d.execute_data("delete from " + user + ".d_dmbdtd where id=" + i_id + " and mabd=" + int.Parse(dataGrid2[i, 0].ToString()));
                d.execute_data("delete from " + user + ".d_dmbdtd where mabd=" + i_id + " and id=" + int.Parse(dataGrid2[i, 0].ToString()));
				load_chitiet();
			}
		...
		{
			dt=d.get_data("select b.mabd,a.ma,trim(a.ten)||' '||a.hamluong as ten,a.dang,c.ten as tenhang,b.soluong from "+user+".d_dmbd a,"+user+".d_dmbdtd b,"+user+".d_dmhang c where a.id=b.mabd and a.mahang=c.id and b.id="+i_id+" order by a.ten").Tables[0];

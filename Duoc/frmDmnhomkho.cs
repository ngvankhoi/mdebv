...
namespace Duoc
{
	...
	public class frmDmnhomkho : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butKetthuc.Location = new System.Drawing.Point(557, 531);
            ...
            this.label2.Location = new System.Drawing.Point(84, 502);
            ...
            this.ten.Location = new System.Drawing.Point(138, 502);
            this.ten.Name = "ten";
            ...
            this.butMoi.Location = new System.Drawing.Point(207, 531);
            ...
            this.butSua.Location = new System.Drawing.Point(277, 531);
            ...
            this.butLuu.Location = new System.Drawing.Point(347, 531);
            ...
            this.butBoqua.Location = new System.Drawing.Point(417, 531);
            ...
            this.butHuy.Location = new System.Drawing.Point(487, 531);
            ...
            this.stt.Location = new System.Drawing.Point(40, 502);
            this.stt.Name = "stt";
            ...
            this.label4.Location = new System.Drawing.Point(408, 502);
            ...
            this.nhomcc.Location = new System.Drawing.Point(456, 502);
            ...
            this.loai.Location = new System.Drawing.Point(656, 502);
            this.loai.Name = "loai";
            ...
            this.label1.Location = new System.Drawing.Point(607, 502);
		...
		private void frmDmnhomkho_Load(object sender, System.EventArgs e)
		{
            ...
            user = d.user; itable = d.tableid("", "d_dmnhomkho");
			...
			loai.ValueMember="ID";
			loai.DataSource=d.get_data("select * from "+user+".d_loaikho order by id").Tables[0];
			load_grid();
			AddGridTableStyle();
			ref_text();
		}
		...
		{
            dt = d.get_data("select a.*,b.ten as tennhom,c.ten as tenloai from " + user + ".d_dmnhomkho a left join " + user + ".d_nhomkho b on a.nhom=b.id left join " + user + ".d_loaikho c on a.loai=c.id order by a.stt").Tables[0];
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            ...
            if (l_id == 0)
            {
                try
                {
                    if (bQuanlychinhanh)
                    {
                        l_id = decimal.Parse(d.i_Chinhanh_hientai.ToString() + d.get_capid(105).ToString().PadLeft(2, '0'));
                    }
                    else
                    {
                        l_id = decimal.Parse(d.get_data("select max(id) from " + user + ".d_dmnhomkho").Tables[0].Rows[0][0].ToString()) + 1;
                    }
                    // hien
            ...
            else
            {
                ...
                d.upd_eve_upd_del(itable, i_userid, "upd", d.fields(user + ".d_dmnhomkho", "id=" + l_id));
            }
			if (!d.upd_dmnhomkho(l_id,ten.Text,int.Parse(nhomcc.SelectedValue.ToString()),int.Parse(stt.Value.ToString()),int.Parse(loai.SelectedValue.ToString())))
			{
				MessageBox.Show(lan.Change_language_MessageText("Không cập nhật thông tin")+" "+this.Text.Trim()+" !",d.Msg);
				return;
			}
			d.ins_thongso(Convert.ToInt16(l_id),45);
			load_grid();
			ena_object(false);
		}
...
lan.Change_language_MessageText("Không cho phép hủy ?"),d.Msg);
			...
			try
			{
				if (d.get_data("select * from "+user+".d_dmkho where nhom="+decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,3].ToString())).Tables[0].Rows.Count!=0)
				{
					MessageBox.Show(
...
lan.Change_language_MessageText("Đồng ý hủy thông tin này !"),d.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                d.upd_eve_upd_del(itable, i_userid, "del", d.fields(user + ".d_dmnhomkho", "id=" + l_id));
				d.execute_data("delete from "+user+".d_dmnhomkho where id="+l_id);
				load_grid();
			}
		}

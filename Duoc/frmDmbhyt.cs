...
namespace Duoc
{
	...
	public class frmDmbhyt : System.Windows.Forms.Form
	{
        ...
        private MaskedTextBox.MaskedTextBox chitra;
		/// <summary>
		...
		{
			dt=d.get_data("select * from "+user+".d_dmbhyt order by stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
				stt.Value=decimal.Parse(d.get_data("select max(stt) from "+user+".d_dmbhyt").Tables[0].Rows[0][0].ToString())+1;
			}
			catch{stt.Value=1;}
            chitra.Text = "";
			ena_object(true);
		...
		{
			if (!d.upd_dmbhyt((chitra.Text!="")?decimal.Parse(chitra.Text.ToString()):0,int.Parse(stt.Value.ToString())))
			{
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				d.execute_data("delete from d_dmbhyt where chitra="+decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,1].ToString()));
				if (!d.bDanhmuc) d.writeXml("d_thongso","c01","1");
				load_grid();
			}
		}
		...
		private void ref_text()
		{
			try
			{
				...
				chitra.Text=dataGrid1[i,1].ToString();
			}
			catch{}
		}

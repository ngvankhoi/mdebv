...
namespace Duoc
{
	...
	/// </summary>
	public class rptNxt_nhombc : System.Windows.Forms.Form
	{
        ...
        private int i_nhom, i_dongiale, i_loai, i_userid=0;
		private bool bClear=true;
		private string user,xxx,sql,s_mmyy,s_tu,s_den,s_yy,s_kho,s_manhom,tenreport,exp,s_khott;
		...
		public rptNxt_nhombc(AccessData acc,int nhom,string kho,string mmyy,int loai, int _userid)
		{
			//
			// Required for Windows Form Designer support
			//
			InitializeComponent();
			d=acc;i_nhom=nhom;s_kho=kho;i_loai=loai;
			tu.Value=decimal.Parse(mmyy.Substring(0,2));
			den.Value=tu.Value;
			yyyy.Value=decimal.Parse("20"+mmyy.Substring(2,2));
			this.Text+=(i_loai==1)?" nhóm báo cáo ":" nhóm dược bệnh viện";
            i_userid = _userid;
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(rptNxt_nhombc));
            ...
            this.butIn.Location = new System.Drawing.Point(162, 222);
            ...
            this.manguon.Location = new System.Drawing.Point(56, 39);
            ...
            this.yyyy.Location = new System.Drawing.Point(208, 15);
            ...
            this.den.Location = new System.Drawing.Point(128, 15);
            ...
            // opt
            // 
            this.opt.Controls.Add(this.rd2);
            this.opt.Controls.Add(this.rd1);
            this.opt.Location = new System.Drawing.Point(56, 177);
            this.opt.Name = "opt";
            this.opt.Size = new System.Drawing.Size(200, 35);
            this.opt.TabIndex = 17;
            this.opt.TabStop = false;
            // 
            // rd2
            // 
            this.rd2.Location = new System.Drawing.Point(122, 10);
            this.rd2.Name = "rd2";
            this.rd2.Size = new System.Drawing.Size(72, 21);
            this.rd2.TabIndex = 1;
            this.rd2.Text = "Chi tiết";
            // 
            // rd1
            // 
            this.rd1.Checked = true;
            this.rd1.Location = new System.Drawing.Point(8, 7);
            this.rd1.Name = "rd1";
            this.rd1.Size = new System.Drawing.Size(72, 25);
            this.rd1.TabIndex = 0;
            this.rd1.TabStop = true;
            this.rd1.Text = "Tổng hợp";
            // 
            // rptNxt_nhombc
            // 
            ...
            this.Name = "rptNxt_nhombc";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Nhập xuất tồn";
            this.MouseMove += new System.Windows.Forms.MouseEventHandler(this.rptNxt_nhombc_MouseMove);
            this.Load += new System.EventHandler(this.rptNxt_nhombc_Load);
		...
		private void rptNxt_nhombc_Load(object sender, System.EventArgs e)
		{
            user = d.user;
            ...
            kho.ValueMember = "ID";
            if (i_loai == 1)
                sql = "select * from " + user + ".d_nhombc where stt<>0 and nhom=" + i_nhom + " order by stt";
            else
                sql = "select * from " + user + ".d_nhombo where stt<>0 and nhom=" + i_nhom + " order by stt";
		...
		private void butIn_Click(object sender, System.EventArgs e)
		{
            ...
            if (i_loai == 2) sql += " from " + user + ".d_dmbd a," + user + ".d_nhombo b where a.nhombo=b.id ";
            else sql += " from " + user + ".d_dmbd a," + user + ".d_nhombc b," + user + ".d_dmnhom c where a.manhom=c.id and c.nhom=b.id";
			sql+=" and a.nhom="+i_nhom+" order by a.id";
		...
		private void get_tondau(string d_mmyy)
		{
            xxx = user + d_mmyy;
			sql="select ";
			sql+="a.mabd,round(t.giamua,"+i_dongiale+") as dongia,t.manguon,sum(a.tondau) as soluong,sum(a.tondau*t.giamua) as sotien ";
			sql+="from "+xxx+".d_tonkhoct a,"+xxx+".d_theodoi t,"+user+".d_dmbd b ";
			sql+=" where a.stt=t.id and a.mabd=b.id and a.tondau<>0 and b.nhom="+i_nhom;
			if (s_kho!="") sql+=" and a.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			if (s_manhom!="") sql+=" and b.nhombo in  ("+s_manhom.Substring(0,s_manhom.Length-1)+")";
			if (manguon.SelectedIndex!=-1) sql+=" and t.manguon="+int.Parse(manguon.SelectedValue.ToString());
			sql+=" group by ";
			sql+="a.mabd,round(t.giamua,"+i_dongiale+"),t.manguon";
			foreach(DataRow r in d.get_data(sql).Tables[0].Rows)
			{
				exp="mabd="+int.Parse(r["mabd"].ToString());
				if (rd2.Checked) exp+=" and dongia="+decimal.Parse(r["dongia"].ToString());
		...
		private void get_chuyennguon(string d_mmyy)
		{
            xxx = user + d_mmyy;
			sql="select ";
			sql+="b.mabd,round(t.giamua,"+i_dongiale+") as dongia, t.manguon,";
			sql+="sum(b.soluong) as soluong,sum(b.soluong*t.giamua) as sotien ";
			sql+=" from "+xxx+".d_chuyenll a,"+xxx+".d_chuyenct b,"+xxx+".d_theodoi t,"+user+".d_dmbd c ";
            sql+=" where a.id=b.id and b.sttt=t.id and b.mabd=c.id and a.nhom="+i_nhom+" and b.soluong>0";
			if (s_kho!="") sql+=" and b.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			if (s_manhom!="") sql+=" and c.nhombo in  ("+s_manhom.Substring(0,s_manhom.Length-1)+")";
			if (manguon.SelectedIndex!=-1) sql+=" and t.manguon="+int.Parse(manguon.SelectedValue.ToString());
			sql+=" group by ";
			sql+="b.mabd,round(t.giamua,"+i_dongiale+"),t.manguon ";
			foreach(DataRow r in d.get_data(sql).Tables[0].Rows)
			{
				exp="mabd="+int.Parse(r["mabd"].ToString());
				if (rd2.Checked) exp+=" and dongia="+decimal.Parse(r["dongia"].ToString());
			...
			sql+="sum(b.soluong) as soluong,sum(b.soluong*t.giamua) as sotien ";
			sql+="from "+xxx+".d_chuyenll a,"+xxx+".d_chuyenct b,"+xxx+".d_theodoi t,"+user+".d_dmbd c ";
            sql+=" where a.id=b.id and b.stttchuyen=t.id and b.mabd=c.id and a.nhom="+i_nhom+" and b.soluong>0";
			if (s_kho!="") sql+=" and b.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			if (s_manhom!="") sql+=" and c.nhombo in  ("+s_manhom.Substring(0,s_manhom.Length-1)+")";
			if (manguon.SelectedIndex!=-1) sql+=" and t.manguon="+int.Parse(manguon.SelectedValue.ToString());
			sql+=" group by ";
		...
		private void get_nhap(string d_mmyy)
		{
            xxx = user + d_mmyy;
			sql="select ";
			sql+="0 as loai,b.mabd,round(b.giamua,"+i_dongiale+") as dongia,a.manguon,";
			sql+="sum(b.soluong) soluong,sum(round(b.sotien+b.sotien*b.vat/100,3)) sotien ";
			sql+="from "+xxx+".d_nhapll a,"+xxx+".d_nhapct b,"+d.user+".d_dmbd c where a.id=b.id and b.mabd=c.id and a.nhom="+i_nhom+" and b.soluong>0";
			if (s_kho!="") sql+=" and a.makho in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			if (s_manhom!="") sql+=" and c.nhombo in  ("+s_manhom.Substring(0,s_manhom.Length-1)+")";
			if (manguon.SelectedIndex!=-1) sql+=" and a.manguon="+int.Parse(manguon.SelectedValue.ToString());
			sql+=" group by ";
			sql+="b.mabd,round(b.giamua,"+i_dongiale+"),a.manguon";
			if (s_khott=="")
			{
				sql+=" union all select ";
				sql+="1 as loai,b.mabd,round(t.giamua,"+i_dongiale+") as dongia, t.manguon,";
				sql+="sum(b.soluong) as soluong,sum(b.soluong*t.giamua) as sotien ";
				sql+="from "+xxx+".d_xuatll a,"+xxx+".d_xuatct b,"+xxx+".d_theodoi t,"+user+".d_dmbd c ";
                sql+=" where a.id=b.id and b.sttt=t.id and b.mabd=c.id and a.nhom="+i_nhom+" and a.loai in ('CK','HT','TH') ";
			...
			else
			{
				...
				//
				sql+=" union all select ";
				sql+="0 as loai,b.mabd,round(t.giamua,"+i_dongiale+") as dongia, t.manguon,";
				sql+="sum(b.soluong) as soluong,sum(b.soluong*t.giamua) as sotien ";
				sql+="from "+xxx+".d_xuatll a,"+xxx+".d_xuatct b,"+xxx+".d_theodoi t,"+user+".d_dmbd c ";
                sql+=" where a.id=b.id and b.sttt=t.id and b.mabd=c.id and a.nhom="+i_nhom+" and a.loai in ('CK','HT','TH') ";
				if (s_kho!="") sql+=" and a.khon in ("+s_kho.Substring(0,s_kho.Length-1)+")";
		...
		private void get_xuat(string d_mmyy)
		{
			...
			sql="select ";
			sql+="b.mabd,round(t.giamua,"+i_dongiale+") as dongia,t.manguon,sum(b.soluong) as soluong,sum(b.soluong*t.giamua) as sotien ";
			sql+="from "+xxx+".d_xuatll a,"+xxx+".d_xuatct b,"+xxx+".d_theodoi t,"+user+".d_dmbd c ";
            sql+=" where a.id=b.id and b.sttt=t.id and b.mabd=c.id and a.nhom="+i_nhom+" and a.loai in ('CK')";
			if (s_kho!="") sql+=" and a.khox in ("+s_kho.Substring(0,s_kho.Length-1)+")";
			if (s_manhom!="") sql+=" and c.nhombo in  ("+s_manhom.Substring(0,s_manhom.Length-1)+")";
			if (manguon.SelectedIndex!=-1) sql+=" and t.manguon="+int.Parse(manguon.SelectedValue.ToString());
			sql+=" and b.soluong>0";
		...
		private void rptNxt_nhombc_MouseMove(object sender, System.Windows.Forms.MouseEventArgs e)
		{
			if (bClear)
			{
...
lan.Change_language_MessageText("Tháng không hợp lệ !"),d.Msg);
			...
			ds.Clear();
			dsxml.Clear();
            sql = "select * from " + user + ".d_dmkho where hide=0 and khott=1 and nhom=" + i_nhom + " order by id";
...
lan.Change_language_MessageText("Không có số liệu !"),d.Msg);
			...
			get_sort();
			tenreport=(rd1.Checked)?"d_bcnxt_nhombc_th.rpt":"d_bcnxt_nhombc.rpt";
			...
			frmReport f=new frmReport(d,dsxml.Tables[0], i_userid,tenreport,s_thoigian,(manguon.SelectedIndex==-1)?"":"Nguồn :"+manguon.Text,"","","","","","","","");
			f.ShowDialog();
		}
	}
}

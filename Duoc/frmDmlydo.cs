...
namespace Duoc
{
	...
	public class frmDmlydo : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.label4.Location = new System.Drawing.Point(489, 502);
            ...
            this.nhomcc.Location = new System.Drawing.Point(537, 502);
		...
		{
            user = d.user; itable = d.tableid("", "d_dmlydotscd");
			dsxml.ReadXml("..\\..\\..\\xml\\d_nhapxuat.xml");
		...
		{
			dt=d.get_data("select a.*,case when a.loai=0 then 'Nhập' else 'Xuất' end as nhapxuat from "+user+".d_dmlydotscd a where a.nhom="+i_nhom+" order by a.loai,a.stt").Tables[0];
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			try
			{
                stt.Value = decimal.Parse(d.get_data("select max(stt) from " + user + ".d_dmlydotscd where nhom=" + i_nhom).Tables[0].Rows[0][0].ToString()) + 1;
			}
			catch{stt.Value=1;}
			l_id=0;
			ten.Text="";
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
            ...
            if (l_id == 0)
            {
                try
                {
                    l_id = int.Parse(d.get_data("select max(id) from " + user + ".d_dmlydotscd").Tables[0].Rows[0][0].ToString()) + 1;
                }
                catch { l_id = 1; }
            ...
            else
            {
                ...
                d.upd_eve_upd_del(itable, i_userid, "upd", d.fields(user + ".d_dmlydotscd", "id=" + l_id));
            }
			if (!d.upd_dmlydotscd(l_id,ten.Text,int.Parse(stt.Value.ToString()),i_nhom,nhomcc.SelectedIndex))
			{
				MessageBox.Show(
lan.Change_language_MessageText("Không cập nhật thông tin")+" "+this.Text.Trim()+" !",d.Msg);
				return;
			}
...
lan.Change_language_MessageText("Đồng ý hủy thông tin này ?"),d.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                d.upd_eve_upd_del(itable, i_userid, "del", d.fields(user + ".d_dmlydotscd", "id=" + l_id));
				d.execute_data("delete from "+user+".d_dmlydotscd where id="+l_id);
				if (!d.bDanhmuc) d.writeXml("d_thongso","c01","1");
				load_grid();
			}
		}

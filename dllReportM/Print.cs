...
namespace dllReportM
{
	...
	public class Print
	{
        ...
        public void Printer1(AccessData m,DataSet _ds, string stt, string tenfile, int kieu)
        {
            try
            {
                ReportDocument oRpt = new ReportDocument();
                ...
                oRpt.DataDefinition.FormulaFields["s_soyte"].Text = "'" + m.Syte.Replace("'", "''") + "'";
                oRpt.DataDefinition.FormulaFields["s_tenbv"].Text = "'" + m.Tenbv.Replace("'", "''") + "'";
                if (kieu != 0)
                {
                    oRpt.PrintOptions.PaperSize = PaperSize.DefaultPaperSize;//.PaperA4;
                    oRpt.PrintOptions.PaperOrientation = (kieu == 1) ? PaperOrientation.Portrait : PaperOrientation.Landscape;

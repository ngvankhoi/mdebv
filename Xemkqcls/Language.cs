...
namespace frmCanlamsan
{
	...
	public class Language
	{
		...
		public void Write_to_Xml(string frmName, Form frm)
		{
			...
			r["vphap"]="";
			ds.Tables[0].Rows.Add(r);
			ds.WriteXml(@"..\..\..\Language\cdha_"+frmName+".xml",XmlWriteMode.WriteSchema);
		}

		public void Changelanguage_to_English(string frmName, Form frm)
		{
            if (File.Exists(@"..\..\..\Language\cdha_" + frmName + ".xml")) ds.ReadXml(@"..\..\..\Language\cdha_" + frmName + ".xml");
            else
            {
                MessageBox.Show("File not found " + @"..\..\..\Language\cdha_" + frmName + ".xml");
                return;
            }
			...
			TabControl tabclt;
            string m_thongso = "0";
            m_thongso = m.Thongso("Ngonngu").ToString();
			flag_language=int.Parse(m_thongso);
		...
		{
			ds.ReadXml(@"..\..\..\Language\cdha_"+frmName+".xml");
			string lbl="";
			foreach(Control ctl in frm.Controls)
			{
				switch (ctl.GetType().ToString())
		...
		public void Read_Language_to_Xml(string frmName,Form frm)
		{
			...
			try
			{
				dsTemp.ReadXml(@"..\..\..\Language\cdha_"+frmName+".xml");

	...
	public class frmChonthongso : System.Windows.Forms.Form
	{
		...
		private void load_makho()
		{
			if (makp.SelectedIndex!=-1 && nhom.SelectedIndex!=-1)
			{
				...
				if (bThua)
				{
                    ...
                    {
                        d.upd_loaiphieu(0,lan.Change_language_MessageText("Hoàn trả thừa theo khoa/phòng"),i_loai,int.Parse(nhom.SelectedValue.ToString()),0,0,false);
                    }
				}
				else sql+=" and id<>0";
				sql+=" order by stt";
        ...
        private void loc_phieu(bool all)
        {
            ...
            if (bThua)
            {
                ...
                if (d.get_data("select * from " + user + ".d_loaiphieu where id=0").Tables[0].Rows.Count == 0)
                {
                    d.upd_loaiphieu(0, "Hoàn trả thừa theo khoa/phòng", i_loai, i_nhom, 0, 0,false);
                }
            }
            else sql += " and id<>0";

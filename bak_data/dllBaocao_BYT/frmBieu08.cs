...
namespace dllBaocao_BYT
{
	...
	public class frmBieu08 : System.Windows.Forms.Form
	{
		...
		public frmBieu08(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu08(string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		private void InitializeComponent()
		{
            ...
            this.butKetthuc.Location = new System.Drawing.Point(522, 500);
		...
		private void frmBieu08_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_08_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_08 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".bieu_08 a,"+m.user+".dm_08 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_08 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_08 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_08 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					if (long.Parse(dataGrid1[i_row,i_col].ToString())>long.Parse(dataGrid1[i_row,3].ToString()) || (long.Parse(dataGrid1[i_row,4].ToString())+long.Parse(dataGrid1[i_row,5].ToString())+long.Parse(dataGrid1[i_row,6].ToString()))>long.Parse(dataGrid1[i_row,3].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}

...
namespace dllBaocao_BYT
{
	...
	public class frmKh133 : System.Windows.Forms.Form
	{
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh133));
            ...
            this.Text = "Biểu 13.3//Thực hiện công tác phòng bệnh";
		...
		private void frmKh133_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_133_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_133 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_133 a,"+m.user+".kh_dm_133 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_133 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_133 where id="+l_id).Tables[0].Rows)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_133 where id="+l_id);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (long.Parse(dataGrid1[i_row,14].ToString())>long.Parse(dataGrid1[i_row,13].ToString()) || long.Parse(dataGrid1[i_row,28].ToString())>long.Parse(dataGrid1[i_row,27].ToString()) || long.Parse(dataGrid1[i_row,30].ToString())>long.Parse(dataGrid1[i_row,29].ToString()) || long.Parse(dataGrid1[i_row,32].ToString())>long.Parse(dataGrid1[i_row,31].ToString()))
				{

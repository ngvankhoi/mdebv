...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh032 : System.Windows.Forms.Form
	{
		...
		public frmKh032(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh032(string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh032));
		...
		private void frmKh03_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_032_dk").Tables[0];
			load_grid(false);
		...
		private void load_grid(bool dm)
		{
            if (dm)
            {
                ds = m.get_data("select * from " + m.user + ".khdm_032 order by to_number(stt)");
                DataRow[] dr = ds.Tables[0].Select("stt='0'");
                if (dr.Length > 0)
                {
                    dr[0]["stt"] = "A";
                }
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_032 a,"+m.user+".khdm_032 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		private void load_value(bool load)
		{
			...
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_032 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_032 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				l_id=long.Parse(cmbNgay.SelectedValue.ToString());
				m.execute_data("delete from "+m.user+".khbieu_032 where id="+l_id);
				load_ngay();
				load_grid(false);
			}
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            DataRow[] r = dt.Select("ma='C" + i_col.ToString().PadLeft(2, '0') + "'");
            if (r.Length != 0)
            {
                ten.Text = r[0][1].ToString();
                dk.Text = r[0][2].ToString();
            }
            else
            {
                ten.Text = "";
            ...
            try
            {
                ...
                {
                    d_tong += decimal.Parse(dr[i][i_col].ToString());
                }
                m.updrec_031(ds.Tables[0], stt, i_col, d_tong);
                if (i_col >= 3 && i_col <= 7 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        void sum(string exp, string stt, int k)
        {
            DataRow[] dr = ds.Tables[0].Select(exp);
            Int16 cRec = Convert.ToInt16(dr.Length);
            Decimal d_tong = 0;
            for (int i = 0; i < cRec; i++)
            {
                d_tong += Decimal.Parse(dr[i][k].ToString());
            }
            m.updrec_031(ds.Tables[0], stt, k, d_tong);
        }
	}
}

...
namespace DllPhonggiuong
{
	public class frmDmpttt : System.Windows.Forms.Form
	{
		...
		public string m_mapt,m_tenpt,m_loaipt;
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmDmpttt));
			...
			this.butChon.Location = new System.Drawing.Point(288, 416);
			...
			this.butKetthuc.Location = new System.Drawing.Point(376, 416);
		...
		private void load_treeView()
		{
			...
			TreeNode node;
			int i;
			foreach(DataRow r in m.get_data("select * from phanloaipttt order by ma desc").Tables[0].Rows)
			{
				i=int.Parse(r["ma"].ToString());
				node=treeView1.Nodes.Add(r["ten"].ToString());
				foreach(DataRow r1 in m.get_data("select * from muc where id_pttt="+i+" order by id_muc").Tables[0].Rows)
				{
		...
		{
			dt=m.get_data("select * from dmpttt where mapt like '"+s_mapt+"%'"+" order by mapt").Tables[0];

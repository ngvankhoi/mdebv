...
namespace DuTruKhoTTB
{
	...
	public class frmChonthongso : System.Windows.Forms.Form
	{
		...
		public int i_makp,i_nhom,i_phieu,i_macstt,i_buoi,i_somay;
		public decimal l_duyet;
		public string s_makp,s_makho,s_ngay,s_tenkp,s_phieu,s_mmyy,s_tennhom,s_manguon,s_nhomkho;
		private DataRow r;
		...
		public frmChonthongso(LibTTB.AccessData acc,int thuoc,int loai,int benhnhan,string makp,bool thua,string _nhomkho)
		{
			//
			...
			ttb=acc;i_thuoc=thuoc;s_nhomkho=_nhomkho;
			i_loai=loai;i_benhnhan=benhnhan;
            s_makp = makp; bThua = thua; if (ttb.bBogo) tv.GanBogo(this, textBox111);
			//
		...
		private void load_makp()
		{
			...
			{
                sql = "select a.* from " + user + ".d_duockp a left join " + user + ".btdkp_bv b on a.makp=b.makp where a.nhomttb like '%" + int.Parse(nhom.SelectedValue.ToString()) + ",%'";
                if (s_makp != "")
                {
		...
		{
            sql = "select * from " + user + ".ttb_dmnhomkho where loai=" + i_thuoc;
		...
		private void load_makho()
		{
			...
			{
                sql=ttb.getkho(i_thuoc,int.Parse(nhom.SelectedValue.ToString()),bThua,dtmakp,int.Parse(makp.SelectedValue.ToString()),i_loai,ngay.Text,dtletet);
				dtkho=ttb.get_data(sql).Tables[0];
				makho.DataSource=dtkho;
                sql = "select * from " + user + ".ttb_loaiphieu where nhom=" + int.Parse(nhom.SelectedValue.ToString()) + " and loai=" + i_loai;
				if (bThua)
				{
                    sql = "select * from " + user + ".ttb_loaiphieu where id=0";
                    if (ttb.get_data("select * from " + user + ".ttb_loaiphieu where id=0").Tables[0].Rows.Count == 0)
						ttb.upd_loaiphieu(0,lan.Change_language_MessageText("Hoàn trả thừa theo khoa/phòng"),i_loai,int.Parse(nhom.SelectedValue.ToString()),0,0);
				}
				else sql+=" and id<>0";
				sql+=" order by stt";
		...
		private void butOk_Click(object sender, System.EventArgs e)
		{
			...
			if (ttb.bKiemtra_duyet(i_nhom)) ttb.upd_tonkho(s_mmyy,i_nhom,0);
		...
		{
            user = ttb.user;
            string ng = ttb.ngayhienhanh_server;
            dtletet = ttb.get_data("select * from "+user+".letet").Tables[0];
            if (ttb.gio_linh != "00:00" || ttb.sGiochunhat != "00:00" || ttb.bPhatthuoc_kho_khoa)
            {
            ...
            kho_phieu = ttb.get_data("select kho from " + user + ".ttb_dmphieu where id=" + i_loai).Tables[0].Rows[0][0].ToString().Trim();
            dtkp = ttb.get_data("select * from "+user+".btdkp_bv").Tables[0];
            ...
            foreach (DataRow r in dt.Rows)
            {
                ...
                sql = "select a.* from " + user + s_mmyy + ".ttb_tonkhoth a, " + user + ".ttb_dmkho b where a.makho=b.id ";
                //sql += " and b.nhom=" + ttb.nhom_duoc;
		...
		private void load_matutruc()
		{
            if (makp.SelectedIndex != -1)
            {
                r = ttb.getrowbyid(dtmakp, "id=" + int.Parse(makp.SelectedValue.ToString()));
                if (r != null)
                {
                    ...
                    if (s != "") sql += "," + s;
                    sql += ") order by id";
                    matutruc.DataSource = ttb.get_data(sql).Tables[0];
		...
		private void ngay_Validated(object sender, System.EventArgs e)
		{
            ...
            if (ttb.gio_linh != "00:00" || ttb.bPhieulinh_chanle || ttb.bPhatthuoc_kho_khoa) load_makho();
		}
		...
		private void kp_Validated(object sender, System.EventArgs e)
		{
			try
			{
				DataRow r=ttb.getrowbyid(dtmakp,"ma='"+kp.Text+"'");
        ...
        private void loc_phieu(bool all)
        {
            ...
            if (!all && ttb.bMmyy(s_mmyy))
            {
                sql = "select phieu from "+user+s_mmyy+".ttb_duyet where nhom=" + i_nhom + " and to_char(ngay,'dd/mm/yyyy')='" + s_ngay.Substring(0,10) + "' and done<>0 ";
                sql += " and loai=" + i_loai + " and makhoa=" + i_makp;
                foreach (DataRow r in ttb.get_data(sql).Tables[0].Rows) s_phieu += r["phieu"].ToString() + ",";
            }
            sql = "select * from "+user+".ttb_loaiphieu where nhom=" + i_nhom + " and loai=" + i_loai;
            if (bThua)
            {
                sql = "select * from "+user+".ttb_loaiphieu where id=0";
                if (ttb.get_data("select * from "+user+".ttb_loaiphieu where id=0").Tables[0].Rows.Count == 0)
                    ttb.upd_loaiphieu(0, "Hoàn trả thừa theo khoa/phòng", i_loai, i_nhom, 0, 0);
            }
            else sql += " and id<>0";

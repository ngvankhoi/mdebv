...
namespace dllVuKeHoach
{
	...
	public class frmKh04 : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmKh04));
			...
			this.butMoi.Location = new System.Drawing.Point(160, 500);
			...
			this.butKetthuc.Location = new System.Drawing.Point(567, 500);
			...
			this.butHuy.Location = new System.Drawing.Point(475, 500);
			...
			this.Text = "Tình hình y tế xã phường";
		...
		private void frmKh04_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
            dt = m.get_data("select * from " + user + ".kh_dm_04_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + user + ".kh_dm_04 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten from " + user + ".kh_bieu_04 a," + user + ".kh_dm_04 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + user + ".kh_dm_04 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_04 where id=" + l_id).Tables[0].Rows)
			{
		...
		{
            cmbNgay.DataSource = m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".kh_bieu_04 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+user+".kh_bieu_04 where id="+l_id);
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				l_id=decimal.Parse(cmbNgay.SelectedValue.ToString());
				m.execute_data("delete from kh_bieu_04 where id="+l_id);
				load_ngay();
				load_grid(false);
			}
		}
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Tên huyện, TX, TP";
			TextCol1.Width = 186;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
...
//				}
		...
		private void Tongcong()
		{
			...
			for(int k=3;k<ds.Tables[0].Columns.Count-1;k++)
			{
				...
				l_tong=0;
				for(Int16 i=1;i<iRec;i++) l_tong+=Decimal.Parse(r[i][k].ToString());

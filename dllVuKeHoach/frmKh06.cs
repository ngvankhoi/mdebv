...
namespace dllVuKeHoach
{
	...
	/// </summary>
	public class frmKh06 : System.Windows.Forms.Form
	{
		...
		public frmKh06(AccessData acc,string hoten,int userid,int mabv)
		{
			//
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmKh06));
			...
			this.butMoi.Location = new System.Drawing.Point(152, 500);
			...
			this.butSua.Location = new System.Drawing.Point(227, 500);
			...
			this.butLuu.Location = new System.Drawing.Point(302, 500);
			...
			this.butBoqua.Location = new System.Drawing.Point(377, 500);
			...
			this.butHuy.Location = new System.Drawing.Point(467, 500);
			...
			this.Text = "Tình hình nhân lực y tế toàn tỉnh/thànhphố";
			this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
			this.Load += new System.EventHandler(this.frmKh06_Load);
		...
		private void frmKh06_Load(object sender, System.EventArgs e)
		{
            user = m.user;
			...
			load_ngay();
            dt = m.get_data("select * from " + user + ".kh_dm_06_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + user + ".kh_dm_06 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten from " + user + ".kh_bieu_06 a," + user + ".kh_dm_06 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + user + ".kh_dm_06 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_06 where id=" + l_id).Tables[0].Rows)
			{
		...
		{
            cmbNgay.DataSource = m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".kh_bieu_06 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
            DataTable dt1 = m.get_data("select * from " + user + ".kh_bieu_06 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
				if (MessageBox.Show(lan.Change_language_MessageText("Bạn có muốn lấy lại thông tin của lần nhập sau cùng không ?"),LibMedi.AccessData.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
				{
					DataRow r1;
					string cot;
					foreach(DataRow r in ds.Tables[0].Rows)
					{
						r1=m.getrowbyid(dt1,"ma="+int.Parse(r["ma"].ToString()));
						if (r1!=null)
						{
							for(int i=1;i<31;i++)
							{
								cot="c"+i.ToString().PadLeft(2,'0');
								r[cot]=r1[cot].ToString();
							}
						}
					}
				}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+user+".kh_bieu_06 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17+c17+c19+c20+c21+c22+c23+c24+c25+c26+c27+c28+c29+c30+c31>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh06(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c29"].ToString()),int.Parse(r[i]["c30"].ToString()),int.Parse(r[i]["c31"].ToString()),i_userid,int.Parse(r[i]["c32"].ToString()),
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				l_id=decimal.Parse(cmbNgay.SelectedValue.ToString());
				m.execute_data("delete from kh_bieu_06 where id="+l_id);
				load_ngay();
				load_grid(false);
			}
		}
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Trình độ chuyên môn";
			TextCol1.Width = 328;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 33);
			TextCol1.MappingName = "C31";
			TextCol1.HeaderText = "C33";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 27);
			TextCol1.MappingName = "C32";
			TextCol1.HeaderText = "C34";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 28);
			TextCol1.MappingName = "C33";
			TextCol1.HeaderText = "C35";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 29);
			TextCol1.MappingName = "C34";
			TextCol1.HeaderText = "C36";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 30);
			TextCol1.MappingName = "C35";
			TextCol1.HeaderText = "C37";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 31);
			TextCol1.MappingName = "C36";
			TextCol1.HeaderText = "C38";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 32);
			TextCol1.MappingName = "C37";
			TextCol1.HeaderText = "C39";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
			...
			TextCol1=new DataGridColoredTextBoxColumn(de, 33);
			TextCol1.MappingName = "C38";
			TextCol1.HeaderText = "C40";
			TextCol1.Width = 39;
			TextCol1.Format="# ### ###";
			TextCol1.Alignment=HorizontalAlignment.Right;
			ts.GridColumnStyles.Add(TextCol1);
		...
		public Color MyGetColorRowCol(int row, int col)
		{
			...
			if (row==i_tong) c=Color.Black;//Color.Red;
			return c;
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col<3) return;
				exp="ma>=51";stt="C";
				if ((i_row>=3 && i_row<=14) || (dataGrid1[i_row,1].ToString()=="B"))
				{
					exp="ma in (3,4,5,6,7,9,11,12,13)";
					stt="B";
				}
				else if ((i_row>=15 && i_row<=16) || (dataGrid1[i_row,1].ToString()=="C"))
				{
					exp="ma=15";
					stt="C";
				}
				else if ((i_row>=17 && i_row<=18) || (dataGrid1[i_row,1].ToString()=="D"))
				{
					exp="ma=17";
					stt="D";
				}
				else if ((i_row>=19 && i_row<=28) || (dataGrid1[i_row,1].ToString()=="E"))
				{
					exp="ma in (19,20,22,24,25,26,27)";
					stt="E";
				}
				else
				{
					exp="ma in (30,32,34,35,36,37)";
					stt="F";
				}
				DataRow[] r=ds.Tables[0].Select(exp);
				Int16 iRec=Convert.ToInt16(r.Length);
				for(Int16 i=0;i<iRec;i++) l_tong+=Decimal.Parse(r[i][i_col].ToString());
				m.updrec_06(ds.Tables[0],stt,i_col,l_tong);
				//ThanhCuong_Không dùng
//				if (decimal.Parse(dataGrid1[i_row,4].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,5].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,6].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,7].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,8].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,9].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,11].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,12].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,13].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,14].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,15].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,16].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,17].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,18].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,19].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,20].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,21].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,22].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,23].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,24].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,25].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,26].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,27].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,28].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,29].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,30].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,31].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString())
//					|| decimal.Parse(dataGrid1[i_row,32].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,33].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()))
//				{
...
//				}
		...
		private void Tongcong()
		{
			...
			{
				sum("ma in (3,4,5,6,7,9,11,12,13)","B",k);
				sum("ma=15","C",k);
				sum("ma=17","D",k);
				sum("ma in (19,20,22,24,25,26,27)","E",k);
				sum("ma in (30,32,34,35,36,37)","F",k);
		...
		private void sum(string exp,string stt,int k)
		{
			...
			m.updrec_06(ds.Tables[0],stt,k,l_tong);
		}
	}
}

...
namespace dllVuKeHoach
{
	...
	public class frmKh01 : System.Windows.Forms.Form
	{
		...
		private void frmKh01_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+user+".khdm_01_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+user+".khdm_01 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+user+".khbieu_01 a,"+user+".khdm_01 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+user+".khdm_01 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+user+".khbieu_01 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+user+".khbieu_01 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
			dt1=m.get_data("select * from "+user+".khbieu_01 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
                    m.execute_data("delete from "+user+".khbieu_01 where id=" + l_id);
        ...
        private void kiemtranhap(int row_index, int col_index)
        {
            ...
            {
                MessageBox.Show("Không hợp lệ !", "Medisoft");
                dataGrid1[row_index, col_index] = 0; return;
            }

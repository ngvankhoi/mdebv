...
namespace dllVuKeHoach
{
	...
	public class frmKh145 : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmKh145));
			...
			this.butMoi.Location = new System.Drawing.Point(145, 500);
			...
			this.butSua.Location = new System.Drawing.Point(222, 500);
			...
			this.butLuu.Location = new System.Drawing.Point(299, 500);
			...
			this.butBoqua.Location = new System.Drawing.Point(376, 500);
			...
			this.butKetthuc.Location = new System.Drawing.Point(562, 500);
		...
		private void frmKh145_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
            dt = m.get_data("select * from " + user + ".kh_dm_1451_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + user + ".kh_dm_1451 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten from " + user + ".kh_bieu_1451 a," + user + ".kh_dm_1451 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + user + ".kh_dm_1451 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_1451 where id=" + l_id).Tables[0].Rows)
			{
		...
		{
            if (dm) ds1 = m.get_data("select * from " + user + ".kh_dm_1452 order by ma");
			else load_data1();
		...
		private void load_data1()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten,b.ten1,b.ten2,b.ten3 from " + user + ".kh_bieu_1452 a," + user + ".kh_dm_1452 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds1=m.get_data(sql);
			}
			else
			{
		...
		{
            ds1 = m.get_data("select * from " + user + ".kh_dm_1452 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_1452 where id=" + l_id).Tables[0].Rows)
			{
		...
		{
            cmbNgay.DataSource = m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".kh_bieu_1451 group by id,ngay order by ngay").Tables[0];
		}

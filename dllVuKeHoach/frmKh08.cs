...
namespace dllVuKeHoach
{
	...
	public class frmKh08 : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmKh08));
			...
			this.Text = "Tình hình hoạt động BHYT";
		...
		private void frmKh08_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
            dt = m.get_data("select * from " + user + ".kh_dm_08_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + user + ".kh_dm_08 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten,b.donvi from " + user + ".kh_bieu_08 a," + user + ".kh_dm_08 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		private void load_value(bool load)
		{
			try
			{
                ds = m.get_data("select * from " + user + ".kh_dm_08 order by ma");
				DataRow[] dr;
                foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_08 where id=" + l_id).Tables[0].Rows)
				{
		...
		{
            cmbNgay.DataSource = m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".kh_bieu_08 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from kh_bieu_08 where id="+l_id);
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Đơn vị tính";
			TextCol1.Width = 100;
			TextCol1.Format="# ### ###";
...
//			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			string m_ma="C"+i_col.ToString().PadLeft(2,'0');
			if(m_ma=="C02") m_ma="c06";
			DataRow r1=m.getrowbyid(dt,"ma='"+m_ma+"'");
			if (r1!=null)
			{

...
namespace dllVuKeHoach
{
	...
	public class Print
	{
		...
		public void Printer(DataSet ds,string tenfile,string c1,string c2,int kieu,string mayin)
		{
			try
			{
				ReportDocument oRpt=new ReportDocument();
		...
		public void Printer(DataSet ds,string tenfile,string c1,string c2,int kieu, int copy)
		{
			try
			{
				ReportDocument oRpt=new ReportDocument();
		...
		public void Printer(AccessData m,DataSet ds,string tenfile,string c1,string c2,string c3,string c4,string c5,string c6,string c7,string c8,string c9,string c10,int kieu,int copy)
		{
			try
			{
				...
				/*
				oRpt.PrintOptions.PaperSize=PaperSize.DefaultPaperSize;
				if (kieu!=-1) oRpt.PrintOptions.PaperOrientation=(kieu==1)?PaperOrientation.Portrait:PaperOrientation.Landscape;
				*/
				oRpt.PrintToPrinter(copy,false,0,0);
				oRpt.Close(); oRpt.Dispose();
			}
		...
		public void Bia(AccessData m,string tenfile,string c1,string c2,string c3,string c4,string c5,string c6)
		{
			try
			{
				ReportDocument oRpt=new ReportDocument();
				oRpt.Load("..\\..\\..\\report\\"+tenfile,OpenReportMethod.OpenReportByTempCopy);
                oRpt.SetDataSource(new System.Data.DataSet());
				oRpt.DataDefinition.FormulaFields["c1"].Text="'"+c1+"'";
				oRpt.DataDefinition.FormulaFields["c2"].Text="'"+c2+"'";
				oRpt.DataDefinition.FormulaFields["c3"].Text="'"+c3+"'";
				oRpt.DataDefinition.FormulaFields["c4"].Text="'"+c4+"'";

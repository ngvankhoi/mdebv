...
namespace dllVuKeHoach
{
	...
	public class frmKh15 : System.Windows.Forms.Form
	{
		...
		{
			System.Resources.ResourceManager resources = new System.Resources.ResourceManager(typeof(frmKh15));
			...
			this.Text = "Tình hình bệnh tật, tử vong tại bệnh viện theo ICD 10 - WHO";
		...
		private void frmKh15_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
            dt = m.get_data("select * from " + user + ".kh_dm_15_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + user + ".dm_11 order by ma asc");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
                sql = "select a.*,b.stt,b.ten,b.icd10 from " + user + ".kh_bieu_15 a," + user + ".dm_11 b where a.ma=b.ma and a.id=" + l_id + " order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + user + ".dm_11 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + user + ".kh_bieu_15 where id=" + l_id).Tables[0].Rows)
			{
				...
				if (dr!=null)
				{
					...
					dr[0]["c01"]=r["c07"].ToString();
					dr[0]["c02"]=r["c08"].ToString();
					dr[0]["c03"]=r["c09"].ToString();
					dr[0]["c04"]=r["c10"].ToString();
					dr[0]["c05"]=r["c11"].ToString();
					dr[0]["c06"]=r["c12"].ToString();
				}
			}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from kh_bieu_15 where id="+l_id);
					...
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh15(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c05"].ToString()),int.Parse(r[i]["c06"].ToString()),i_userid,int.Parse(r[i]["c07"].ToString()),
							int.Parse(r[i]["c08"].ToString()),int.Parse(r[i]["c09"].ToString()),int.Parse(r[i]["c10"].ToString()),
							int.Parse(r[i]["c11"].ToString()),int.Parse(r[i]["c12"].ToString()));
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		{
            cmbNgay.DataSource = m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from " + user + ".kh_bieu_15 group by id,ngay order by ngay").Tables[0];
		}

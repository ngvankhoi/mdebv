...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh02 : System.Windows.Forms.Form
	{
		...
		public frmKh02(AccessData acc,string hoten,int userid,int mabv)
		{
			//
            ...
            m = acc; tv.GanBogo(this, textBox111);
			s_userid=hoten;
			i_userid=userid;
			i_mabv=mabv;
			this.Text+="//"+s_userid;
        ...
        public frmKh02( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh02));
            ...
            this.butMoi.Location = new System.Drawing.Point(181, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(72, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(253, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(325, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(541, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(469, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(72, 25);
            ...
            this.Text = "Thông tin về sinh, tử";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh02_Load);
		...
		private void frmKh02_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_02_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_02 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_02 a,"+m.user+".khdm_02 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_02 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_02 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_02 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_02 where id="+l_id);
                    m.execute_data("delete from khbieu_02 where id=" + l_id);
                    string exp = "c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16 >0";
                    DataRow[] r = ds.Tables[0].Select(exp);
                    Int16 cRec = Convert.ToInt16(r.Length);
                    for (int i = 0; i < cRec; i++)
                    {
                        m.upd_kh02(l_id, int.Parse(r[i]["ma"].ToString()), ngay.Text, int.Parse(r[i]["c01"].ToString()), int.Parse(r[i]["c02"].ToString()), int.Parse(r[i]["c03"].ToString()),
                            int.Parse(r[i]["c04"].ToString()), int.Parse(r[i]["c05"].ToString()), int.Parse(r[i]["c06"].ToString()), int.Parse(r[i]["c07"].ToString()), int.Parse(r[i]["c08"].ToString()),
                            int.Parse(r[i]["c09"].ToString()), int.Parse(r[i]["c10"].ToString()), int.Parse(r[i]["c11"].ToString()), int.Parse(r[i]["c12"].ToString()), int.Parse(r[i]["c13"].ToString()),
                            int.Parse(r[i]["c14"].ToString()), int.Parse(r[i]["c15"].ToString()), int.Parse(r[i]["c16"].ToString()), i_userid);
                    }
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Tên xã,thị trấn,phường thuộc huyện";
			TextCol1.Width = 186;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		{
			Color c = Color.Black;
            if (this.dataGrid1[row,1].ToString().Trim()=="A")
			{
				c=Color.Red;
				i_tong=row;
			}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            DataRow row = m.getrowbyid(dt, "ma='C" + i_col.ToString().PadLeft(2, '0') + "'");
            if (row != null)
            {
                ten.Text = row["ten"].ToString();
                dk.Text = row["dk"].ToString();
            }
            else
            {
                ten.Text = "";
            ...
            try
            {
                ...
                {
                    l_tong += Decimal.Parse(dr[k][i_col].ToString());
                }
                m.updrec_05(ds.Tables[0], stt, i_col, l_tong);

                if (i_col >= 3 && i_col <= 18 && dataGrid1[i_row, i_col].ToString() != "")
                {
                    kiemtranhap(i_col, i_row);
                }
            ...
            catch
            {
                try
                {
                    ...
                    Int16 cRec = Convert.ToInt16(r.Length);
                    for (int i = 0; i < cRec; i++)
                        l_tong += Decimal.Parse(r[i][i_col].ToString());
                    m.updrec_05(ds.Tables[0], stt, i_col, l_tong);
        ...
        private void kiemtranhap(int col, int row)
        {
            decimal tt = decimal.Parse(dataGrid1[row, 4].ToString()) + decimal.Parse(dataGrid1[row, 5].ToString()) + decimal.Parse(dataGrid1[row, 6].ToString()) + decimal.Parse(dataGrid1[row, 7].ToString()) + decimal.Parse(dataGrid1[row, 8].ToString()) + decimal.Parse(dataGrid1[row, 9].ToString());
            decimal tc = decimal.Parse(dataGrid1[row, 11].ToString()) + decimal.Parse(dataGrid1[row, 12].ToString()) + decimal.Parse(dataGrid1[row, 13].ToString()) + decimal.Parse(dataGrid1[row, 14].ToString()) + decimal.Parse(dataGrid1[row, 15].ToString()) + decimal.Parse(dataGrid1[row, 16].ToString()) + decimal.Parse(dataGrid1[row, 17].ToString()) + decimal.Parse(dataGrid1[row, 18].ToString());
            if (decimal.Parse(dataGrid1[row, col].ToString()) < 0)
            {
                MessageBox.Show("Không chấp nhận số âm !", LibMedi.AccessData.Msg);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 3].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ ! C01 >=0", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
            }
            if (decimal.Parse(dataGrid1[row, 4].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0 || decimal.Parse(dataGrid1[row, 5].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 6].ToString()) < 0 || decimal.Parse(dataGrid1[row, 6].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0 || decimal.Parse(dataGrid1[row, 7].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || decimal.Parse(dataGrid1[row, 9].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()) || decimal.Parse(dataGrid1[row, 3].ToString()) < tt)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 10].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 11].ToString()) < 0 || decimal.Parse(dataGrid1[row, 11].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 12].ToString()) < 0 || decimal.Parse(dataGrid1[row, 12].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 13].ToString()) < 0 || decimal.Parse(dataGrid1[row, 13].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 14].ToString()) < 0 || decimal.Parse(dataGrid1[row, 14].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 15].ToString()) < 0 || decimal.Parse(dataGrid1[row, 15].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 16].ToString()) < 0 || decimal.Parse(dataGrid1[row, 16].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 17].ToString()) < 0 || decimal.Parse(dataGrid1[row, 17].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 18].ToString()) < 0 || decimal.Parse(dataGrid1[row, 18].ToString()) > decimal.Parse(dataGrid1[row, 10].ToString()) || decimal.Parse(dataGrid1[row, 10].ToString()) < tc)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;

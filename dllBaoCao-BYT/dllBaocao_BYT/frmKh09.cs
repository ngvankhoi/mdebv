...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh09 : System.Windows.Forms.Form
	{
		...
		public frmKh09(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh09( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh09));
            ...
            this.butSua.Location = new System.Drawing.Point(234, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(404, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(574, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(489, 500);
            ...
            this.Text = "Tình hình sức khỏe trẻ em";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh09_Load);
		...
		private void frmKh09_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
            dt = m.get_data("select * from " + m.user + ".khdm_09_dk").Tables[0];
			load_grid(false);
		...
		{
            if (dm) ds = m.get_data("select * from " + m.user + ".khdm_09 order by ma");
            else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_09 a,"+m.user+".khdm_09 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
            ds = m.get_data("select * from " + m.user + ".khdm_09 order by ma");
			DataRow[] dr;
            foreach (DataRow r in m.get_data("select * from " + m.user + ".khbieu_09 where id=" + l_id).Tables[0].Rows) 
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_09 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_09 where id="+l_id);
					...
					{
                        m.upd_kh09(l_id, int.Parse(r[i]["ma"].ToString()), ngay.Text, int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c08"].ToString()), i_userid, r[i]["ghichu"].ToString());
					}
				}
				ena_object(false);
				decimal id=l_id;
        ...
        {
            for (int k = 3; k < ds.Tables[0].Columns.Count-1; k++)//-1: bo bot cot ghi chu
            {

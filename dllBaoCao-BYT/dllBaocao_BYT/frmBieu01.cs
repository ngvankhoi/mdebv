...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmBieu01 : System.Windows.Forms.Form
	{
		...
		public frmBieu01(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu01(string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu01));
            ...
            this.butMoi.Location = new System.Drawing.Point(209, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(270, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(331, 500);
           ...
           // this.butKetthuc.Image = global::dllBaocao_BYT.Properties.Resources.exit1;
            ...
            this.butKetthuc.Location = new System.Drawing.Point(514, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(453, 500);
            ...
            this.Text = "Tình hình cán bộ, công chức, viên chức";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu01_Load);
		...
		private void frmBieu01_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_01_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_01 order by loai,kp,ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten,b.loai from "+m.user+".bieu_01 a,"+m.user+".dm_01 b where a.ma=b.ma and a.id="+l_id+" order by b.loai,b.kp,a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_01 order by loai,kp,ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_01 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_01 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
			dt1=m.get_data("select * from "+m.user+".bieu_01 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_01 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13";
					exp+="+c14+c15+c16+c17+c18+c19+c20+c21+c22+c23>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu01(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c23"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col<3) return;
				string exp,stt;
				if (dataGrid1[i_row,26].ToString()=="B")
				{
					exp="ma>=4 and ma<=11";
					stt="B";
				}
				else if (dataGrid1[i_row,26].ToString()=="C")
				{
					exp="ma>51";
					stt="C";
				}
				...
				{
					if (dataGrid1[i_row,i_col].ToString().IndexOf("-")!=-1 || decimal.Parse(dataGrid1[i_row,i_col].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()))
					{
		...
		private void Tongcong()
		{
			...
			{
				if (ds.Tables[0].Columns[k].ColumnName.Substring(0,1)=="C")
				{
					sum("ma>=4 and ma<=11","B",k);
					sum("ma>51","C",k);
					sum("ma>=42 and ma<=51","D",k);
				}
			}
		}

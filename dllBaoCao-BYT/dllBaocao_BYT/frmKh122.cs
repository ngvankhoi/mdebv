...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh122 : System.Windows.Forms.Form
	{
		...
		public frmKh122(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh122( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh122));
            ...
            this.butMoi.Location = new System.Drawing.Point(151, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(84, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(235, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(84, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(319, 500);
            this.butLuu.Name = "butLuu";
            this.butLuu.Size = new System.Drawing.Size(84, 25);
            ...
            this.butBoqua.Location = new System.Drawing.Point(403, 500);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(84, 25);
            ...
            this.butHuy.Location = new System.Drawing.Point(487, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(84, 25);
            ...
            this.Text = "Biểu 12.2 Hoạt động khám chữa bệnh";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh122_Load);
		...
		private void frmKh122_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_122_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_122 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_122 a,"+m.user+".khdm_122 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_122 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_122 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_122 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_122 where id="+l_id);
					...
					{
						m.upd_kh122(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
        ...
        void kiemtranhap(int col, int row)
        {
            ...
            if (decimal.Parse(dataGrid1[row, 4].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) + decimal.Parse(dataGrid1[row, 5].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
            ...
            if (decimal.Parse(dataGrid1[row, 5].ToString()) < 0 || decimal.Parse(dataGrid1[row, 4].ToString()) + decimal.Parse(dataGrid1[row, 5].ToString()) > decimal.Parse(dataGrid1[row, 3].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            ...
            if (decimal.Parse(dataGrid1[row, 7].ToString()) < 0 || decimal.Parse(dataGrid1[row, 7].ToString()) + decimal.Parse(dataGrid1[row, 8].ToString()) > decimal.Parse(dataGrid1[row, 6].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) + decimal.Parse(dataGrid1[row, 7].ToString()) > decimal.Parse(dataGrid1[row, 6].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;

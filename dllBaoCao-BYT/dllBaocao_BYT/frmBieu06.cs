...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmBieu06 : System.Windows.Forms.Form
	{
		...
		private int i_userid,i_mabv,i_tong,i_col=0,i_row=0;
		...
		public frmBieu06(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu06( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu06));
            ...
            this.Load += new System.EventHandler(this.frmBieu06_Load);
		...
		private void frmBieu06_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_06_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_06 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten,b.donvi from "+m.user+".bieu_06 a,"+m.user+".dm_06 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_06 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_06 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_06 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_06 where id="+l_id);
					string exp="c01+c02>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu06(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							int.Parse(r[i]["c02"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				//i_row=dataGrid1.CurrentCell.RowNumber;
				if (i_col<4) return;
				string exp="ma>=26",stt="E";
				if ((i_row >=1 && i_row<=6) || (dataGrid1[i_row,1].ToString()=="A"))
				{
					exp="ma>=2 and ma<=6";
					stt="A";
				}
				else if ((i_row>=7 && i_row<=13) || (dataGrid1[i_row,1].ToString()=="B"))
				{
					exp="ma>=9 and ma<=14";
					stt="B";
				}
				else if ((i_row>=14 && i_row<=18) || (dataGrid1[i_row,1].ToString()=="C"))
				{
					exp="ma>=17 and ma<=20";
					stt="C";
				}
				else if ((i_row>=19 && i_row<=20) || (dataGrid1[i_row,1].ToString()=="D"))
				{
					exp="ma=23";
					stt="D";
				}
				...
				if (dataGrid1[i_row,i_col].ToString()!="") //i_col!=4 && 
				{
					if (dataGrid1[i_row,i_col].ToString().IndexOf("-")!=-1 || decimal.Parse(dataGrid1[i_row,5].ToString())>decimal.Parse(dataGrid1[i_row,4].ToString()))
					{
		...
		private void Tongcong()
		{
			for(int k=4;k<ds.Tables[0].Columns.Count;k++)
			{
				...
				sum("ma>=9 and ma<=14","B",k);
				sum("ma>=17 and ma<=20","C",k);
				sum("ma=23","D",k);
				sum("ma>=26","E",k);
			}
		}

...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh05 : System.Windows.Forms.Form
	{
		...
		public frmKh05(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh05( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh05));
            ...
            this.butMoi.Location = new System.Drawing.Point(140, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(86, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(226, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(86, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(312, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(398, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(570, 500);
            this.butKetthuc.Name = "butKetthuc";
            this.butKetthuc.Size = new System.Drawing.Size(96, 25);
            ...
            this.butHuy.Location = new System.Drawing.Point(484, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(86, 25);
            ...
            this.Text = "Tình hình cơ sở y tế và giường bệnh";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh05_Load);
		...
		private void frmKh05_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_05_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_05 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_05 a,"+m.user+".khdm_05 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_05 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_05 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_05 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
                    m.execute_data("delete from khbieu_05 where id=" + l_id);
                    string exp = "c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14>0";
                    DataRow[] r = ds.Tables[0].Select(exp);
                    Int16 iRec = Convert.ToInt16(r.Length);
                    for (Int16 i = 0; i < iRec; i++)
                    {
                        m.upd_kh05_moi(l_id, int.Parse(r[i]["ma"].ToString()), ngay.Text, int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c14"].ToString()), i_userid);
                    }
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				l_id=decimal.Parse(cmbNgay.SelectedValue.ToString());
				m.execute_data("delete from "+m.user+".khbieu_05 where id="+l_id);
				load_ngay();
				load_grid(false);
			}
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                if (i_col >= 3 && i_col <= 16 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        {
            decimal ts = decimal.Parse(dataGrid1[row, 9].ToString()) + decimal.Parse(dataGrid1[row, 10].ToString()) + decimal.Parse(dataGrid1[row, 11].ToString()) + decimal.Parse(dataGrid1[row, 12].ToString()) + decimal.Parse(dataGrid1[row, 13].ToString()) + decimal.Parse(dataGrid1[row, 14].ToString());
            ...
            if (decimal.Parse(dataGrid1[row, 6].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            ...
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) < ts)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            ...
            if (decimal.Parse(dataGrid1[row, 12].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;
            ...
            if (decimal.Parse(dataGrid1[row, 14].ToString()) < 0)
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0; return;

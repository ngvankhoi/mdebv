...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh01 : System.Windows.Forms.Form
	{
		...
		public frmKh01(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh01( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh01));
            ...
            this.butMoi.Location = new System.Drawing.Point(112, 521);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(89, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(201, 521);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(89, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(290, 521);
            this.butLuu.Name = "butLuu";
            this.butLuu.Size = new System.Drawing.Size(89, 25);
            ...
            this.butBoqua.Location = new System.Drawing.Point(379, 521);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(89, 25);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(557, 521);
            ...
            this.butHuy.Location = new System.Drawing.Point(468, 521);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(89, 25);
            ...
            this.Text = "Đơn vị hành chính, dân số và tình hình sinh tử";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh01_Load);
		...
		private void frmKh01_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_01_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_01 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_01 a,"+m.user+".khdm_01 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_01 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_01 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_01 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
			dt1=m.get_data("select * from "+m.user+".khbieu_01 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
                    ...
                    string exp = "c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17+c18>0";
                    ...
                    for (int i = 0; i < iRec; i++)
                    {
                        m.upd_kh01(l_id, r[i]["ma"].ToString(), ngay.Text, int.Parse(r[i]["C01"].ToString()), int.Parse(r[i]["C02"].ToString()), int.Parse(r[i]["C03"].ToString()),
                            ...
                            int.Parse(r[i]["C13"].ToString()), int.Parse(r[i]["C14"].ToString()), int.Parse(r[i]["C15"].ToString()), int.Parse(r[i]["C16"].ToString()), i_userid, int.Parse(r[i]["C17"].ToString()), int.Parse(r[i]["C18"].ToString()));
                    }
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			{
				l_id=decimal.Parse(cmbNgay.SelectedValue.ToString());
				m.execute_data("delete from "+m.user+".khbieu_01 where id="+l_id);
				load_ngay();
				load_grid(false);
			}
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                i_col = dataGrid1.CurrentCell.ColumnNumber;
                if (i_col < 3) return;
                string exp = "ma >0", stt = "A";
                DataRow[] row = ds.Tables[0].Select(exp);
                Int16 iRec = Convert.ToInt16(row.Length);
                for (int i = 0; i < iRec; i++)
                    s_tong += Decimal.Parse(row[i][i_col].ToString());
                m.updrec_02(ds.Tables[0], stt, i_col, s_tong);
                if (i_col >= 3 && i_col <= 20 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        #region Kiem tra du lieu nhap tren datagrid
        private void kiemtranhap(int row_index, int col_index)
        {
            if (decimal.Parse(dataGrid1[row_index, col_index].ToString()) < 0)
            {
                MessageBox.Show("Không chấp nhận số âm !", LibMedi.AccessData.Msg);
                dataGrid1[row_index, col_index] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row_index, 6].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()) || (decimal.Parse(dataGrid1[row_index, 6].ToString()) + decimal.Parse(dataGrid1[row_index, 7].ToString()) + decimal.Parse(dataGrid1[row_index, 8].ToString()) + decimal.Parse(dataGrid1[row_index, 9].ToString())) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show(lan.Change_language_MessageText(lan.Change_language_MessageText("Không hợp lê !")), LibMedi.AccessData.Msg);
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 7].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 8].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 9].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 10].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 11].ToString()) > decimal.Parse(dataGrid1[row_index, 10].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 12].ToString()) > decimal.Parse(dataGrid1[row_index, 5].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 13].ToString()) > decimal.Parse(dataGrid1[row_index, 12].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 14].ToString()) > decimal.Parse(dataGrid1[row_index, 12].ToString()) || (decimal.Parse(dataGrid1[row_index, 14].ToString()) + decimal.Parse(dataGrid1[row_index, 16].ToString()) + decimal.Parse(dataGrid1[row_index, 18].ToString())) > decimal.Parse(dataGrid1[row_index, 12].ToString()))
            {
                MessageBox.Show("Không hợp lệ !  ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 15].ToString()) > decimal.Parse(dataGrid1[row_index, 14].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 16].ToString()) > decimal.Parse(dataGrid1[row_index, 12].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 17].ToString()) > decimal.Parse(dataGrid1[row_index, 16].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
            if (decimal.Parse(dataGrid1[row_index, 18].ToString()) > decimal.Parse(dataGrid1[row_index, 12].ToString()))
            {
                MessageBox.Show("Không hợp lệ ! ", "Medisoft 2003");
                dataGrid1[row_index, col_index] = 0; return;
            }
        ...
        private void tongcong()
        {
            ...
            for (int k = 3; k < ds.Tables[0].Columns.Count; k++)
            {
                ...
                for (int i = 0; i < iRec; i++)
                {
                    d_tong += Decimal.Parse(r[i][k].ToString());
                }
                m.updrec_02(ds.Tables[0], stt, k, d_tong);
            }
        }
        #endregion
	}

...
namespace dllBaocao_BYT
{
	...
	/// </summary>
	public class frmKh121 : System.Windows.Forms.Form
	{
		...
		public frmKh121(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh121( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh121));
            ...
            this.butMoi.Location = new System.Drawing.Point(168, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(320, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(548, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(472, 500);
            ...
            this.Text = "Biểu 12.1//Công tác khám chữa bệnh và dịch vụ y tế";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh121_Load);
		...
		private void frmKh121_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".khdm_121_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".khdm_121 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".khbieu_121 a,"+m.user+".khdm_121 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".khdm_121 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".khbieu_121 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".khbieu_121 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".khbieu_121 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh121(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c08"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
            ...
            try
            {
                ...
                if (i_col >= 3 && i_col <= 10 && dataGrid1[i_row, i_col].ToString() != "")
                {
        ...
        void kiemtranhap(int col, int row)
        {
            ...
            if (decimal.Parse(dataGrid1[row, 8].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) + decimal.Parse(dataGrid1[row, 9].ToString()) > decimal.Parse(dataGrid1[row, 7].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);
                dataGrid1[row, col] = 0;
                return;
            }
            if (decimal.Parse(dataGrid1[row, 9].ToString()) < 0 || decimal.Parse(dataGrid1[row, 8].ToString()) + decimal.Parse(dataGrid1[row, 9].ToString()) > decimal.Parse(dataGrid1[row, 7].ToString()))
            {
                MessageBox.Show("Không hợp lệ !", LibMedi.AccessData.Msg, MessageBoxButtons.OK, MessageBoxIcon.Error);

...
namespace Medisoft
{
    public class frmKhambenh_ba : System.Windows.Forms.Form
    {
                    ...
                    if (rbv != null)
                    {
                        ...
                        if (s_ngoaidm == "1")
                        {
                                 ...
                                 MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button1) == DialogResult.Yes)
                            {
                                ...
                                {
                                    if (MessageBox.Show("Bạn muốn chuyển sang bệnh viện ngoài danh mục thì phải có lý do", "Medisoft", MessageBoxButtons.OKCancel, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1) == DialogResult.OK)
                                    {
        ...
        private void buttonkho_Click(object sender, EventArgs e)
        {
            ...
            sql = "select mmyy from " + user + ".table order by substr(mmyy,3,2) desc,substr(mmyy,1,2) desc";
            DataTable dt = m.get_data(sql).Tables[0];
            sql = "select kho from " + user + ".d_dmphieu where id in(5,6,7) ";
            DataSet ads = m.get_data(sql);
            string tmp_makho = "";
            foreach (DataRow dr in ads.Tables[0].Rows)
            {
                tmp_makho += dr["kho"].ToString().Trim(',') + ",";
            }
            foreach (DataRow r in dt.Rows)
            {
                s_mmyy = r["mmyy"].ToString();
                //if (d.get_data(s_mmyy,"select a.* from d_tonkhoth a,"+d.user+".d_dmkho b where a.makho=b.id and b.nhom="+m.nhom_duoc).Tables[0].Rows.Count>0) break;
                //TU:28/03/2011
                sql = "select a.* from " + d.user + s_mmyy + ".d_tonkhoth a," + d.user + ".d_dmkho b ";
                sql += " where a.makho=b.id and b.hide=0 and b.nhom=" + m.nhom_duoc;
                if (tmp_makho.Replace(",,", ",").Trim().Trim(',') != "") sql += " and b.id in(" + tmp_makho.Replace(",,", ",").Trim().Trim(',') + ")";
                if (d.get_data(sql).Tables[0].Rows.Count > 0) break;
            }
            frmXemtonth f = new frmXemtonth(s_mmyy, (madoituong.Text == "1") ? 5 : 6, m.nhom_duoc, (madoituong.Text == "") ? 0 : int.Parse(madoituong.Text), ngayvv.Text, tmp_makho , true);
            f.ShowDialog();
        }
        ...
        private void butTiemchung_Click(object sender, EventArgs e)
        {
            ...
            if (chkTudongchonthongso.Checked)
            {
                ...
                dllvpkhoa_chidinh.frmGoiDichvu f = new dllvpkhoa_chidinh.frmGoiDichvu(1, int.Parse(drttKho["id"].ToString()), ngayvv.Text.Substring(0, 10), 2, int.Parse(dtPhieu.Rows[0]["id"].ToString()), m.mmyy(ngayvv.Text), drttKho["makho"].ToString(), s_manguon, l_duyet1, s_mabn,
                       ...
                       i_maba, false, false, 0, 1, "xxx.benhanpk");
                f.ShowInTaskbar = false;
                f.ShowDialog();
            }
            else

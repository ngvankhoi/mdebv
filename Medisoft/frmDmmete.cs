...
namespace Medisoft
{
	...
	/// </summary>
	public class frmDmmete : System.Windows.Forms.Form
	{
		...
		public frmDmmete(AccessData acc)
		{
			//
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDmmete));
            ...
            this.ten.Location = new System.Drawing.Point(128, 312);
            this.ten.Name = "ten";
            ...
            this.Text = "Danh mục phương pháp vô cảm";
            this.Load += new System.EventHandler(this.frmDmmete_Load);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmDmmete_KeyDown);
		...
		private void frmDmmete_Load(object sender, System.EventArgs e)
		{
            user = m.user;
			load_grid();
			AddGridTableStyle();
		...
		{
			dt=m.get_data("select * from "+user+".dmmete order by ma").Tables[0];
		...
		{
			if (!kiemtra()) return ;
			if (!m.upd_dmmete(ma.Text,ten.Text))
			{
				MessageBox.Show(lan.Change_language_MessageText("Không cập nhật được thông tin phương pháp vô cảm !"));
				return;
			}
			load_grid();
			ena_object(false);
		...
		{
            //if (m.get_data("select * from pttt where phuongphap='"+ma.Text+"'").Tables[0].Rows.Count!=0)
            //{
            //    MessageBox.Show(lan.Change_language_MessageText("Phương pháp vô cảm đã sử dụng không thể hủy !"),LibMedi.AccessData.Msg);
            //    return;
            //}
			if (dt.Rows.Count==1)
			{
			...
			{
                m.execute_data("delete from " + user + ".dmmete where ma='" + ma.Text + "'");
				load_grid();
			}
		}
		...
		private void ma_Validated(object sender, System.EventArgs e)
		{
			try
			{
                if (m.get_data("select * from " + user + ".dmmete where ma='" + ma.Text + "'").Tables[0].Rows.Count != 0)
				{
		...
		private void frmDmmete_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{

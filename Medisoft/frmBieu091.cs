...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu091 : System.Windows.Forms.Form
	{
		...
		private int i_userid,i_mabv,i_row=0,i_col;
		...
		public frmBieu091(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu091( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu091));
            ...
            this.Name = "frmBieu091";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Hoạt động chỉ đạo tuyến";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu091_Load);
		...
		private void frmBieu091_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_091 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten,b.donvi from "+m.user+".bieu_091 a,"+m.user+".dm_091 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_091 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_091 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_091 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_091 where id="+l_id);
					string exp="tongso>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu091(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["tongso"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;

...
namespace Medisoft
{
	...
	/// </summary>
	public class frmKh144 : System.Windows.Forms.Form
	{
		...
		public frmKh144(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh144( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh144));
            ...
            this.butMoi.Size = new System.Drawing.Size(75, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(236, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(75, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(311, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(536, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(461, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(75, 25);
            ...
            this.Text = "Biểu 14.4//Các bệnh lây và các bệnh quan trọng";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh144_Load);
		...
		private void frmKh144_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_144_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_144 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_144 a,"+m.user+".kh_dm_144 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_144 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_144 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_144 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_144 where id="+l_id);
					...
					{
						m.upd_kh144(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c26"].ToString()),int.Parse(r[i]["c27"].ToString()),int.Parse(r[i]["c28"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (decimal.Parse(dataGrid1[i_row,4].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) 
					...
					|| decimal.Parse(dataGrid1[i_row,7].ToString())>decimal.Parse(dataGrid1[i_row,5].ToString()) 
					|| decimal.Parse(dataGrid1[i_row,8].ToString())>decimal.Parse(dataGrid1[i_row,6].ToString()) 
					...
					|| decimal.Parse(dataGrid1[i_row,11].ToString())>decimal.Parse(dataGrid1[i_row,9].ToString()) 
					|| decimal.Parse(dataGrid1[i_row,12].ToString())>decimal.Parse(dataGrid1[i_row,10].ToString()) 

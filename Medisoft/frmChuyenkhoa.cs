...
{
	/// <summary>
	/// Summary description for frmChuyenkhoa.
	/// </summary>
	public class frmChuyenkhoa : System.Windows.Forms.Form
	{
		...
		private string user,m_mabn,m_hoten,m_namsinh,m_makp,sql,m_mabs,m_tuoivao,m_giuong,m_ngay,m_chandoan,m_maicd,s_maicd,s_ngay,m_ngayra,m_khoacuoi,m_tenkhoacuoi,m_ngayvv;
		private decimal m_maql,m_id,m_mavaovien;
		...
		public frmChuyenkhoa(AccessData acc,string mabn,string hoten,string namsinh,string makp,decimal maql,int userid,int maba,string mabs,string tuoivao,string giuong,string ngay,string ngayra,string khoacuoi,string tenkhoacuoi,string ngayvv,decimal mavaovien)
		{
			//
			...
			m=acc;m_mabn=mabn;m_hoten=hoten;m_namsinh=namsinh;m_makp=makp;m_maql=maql;m_userid=userid;
			m_maba=maba;m_mabs=mabs;m_tuoivao=tuoivao;m_giuong=giuong;m_ngay=ngay;m_ngayra=ngayra;
            m_khoacuoi = khoacuoi; m_tenkhoacuoi = tenkhoacuoi; m_ngayvv = ngayvv; m_mavaovien = mavaovien;
		...
		private void frmChuyenkhoa_Load(object sender, System.EventArgs e)
		{
            ...
            tenkp.DataSource = m.get_data("select * from " + user + ".btdkp_bv where loai=0 order by makp").Tables[0];
		...
		private void load_grid()
		{
            ...
            sql += " c.tenkp as tenkhoaden,a.chandoan,a.maicd,a.makp,a.khoaden,a.id from " + user + ".chuyenkhoa a," + user + ".btdkp_bv b," + user + ".btdkp_bv c";
		...
		{
            tenkhoaden.DataSource = m.get_data("select * from " + user + ".btdkp_bv where makp<>'01' and loai=0 and makp<>'" + tenkp.SelectedValue.ToString() + "'" + " order by makp").Tables[0];
		}
		...
		private void butSua_Click(object sender, System.EventArgs e)
		{
			try
			{
				m_id=decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,5].ToString());
                m_ngay = m.get_data("select to_char(ngay,'dd/mm/yyyy hh24:mi') as ngay from " + user + ".nhapkhoa where id=" + m_id).Tables[0].Rows[0][0].ToString();
			}
			catch{}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			m.upd_nhapkhoa(m_id,m_mabn,m_mavaovien,m_maql,khoaden.Text,m_maba,m_ngayvv,ngay.Text,m_tuoivao,m_giuong,makp.Text,chandoan.Text,maicd.Text,m_mabs,1,m_userid);
			ena_object(false);
			load_grid();
			upd=true;
		}
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (MessageBox.Show(lan.Change_language_MessageText("Đồng ý hủy thông tin đang chọn !"),LibMedi.AccessData.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
				{
					m_id=decimal.Parse(dataGrid1[dataGrid1.CurrentCell.RowNumber,5].ToString());
					m.delrec(dt,"id="+m_id);
                    m.execute_data("delete from " + user + ".chuyenkhoa where id=" + m_id);
					load_grid();
				}
			}
			catch{}
		...
		private void butKetthuc_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (m_khoacuoi!="")
				{
                    if (m.get_data("select khoaden from " + user + ".chuyenkhoa where maql=" + m_maql + " order by ngaychuyen desc").Tables[0].Rows[0][0].ToString() != m_khoacuoi)
					{

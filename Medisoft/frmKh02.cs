...
namespace Medisoft
{
	...
	public class frmKh02 : System.Windows.Forms.Form
	{
		...
		private void frmKh02_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_02_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_02 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_02 a,"+m.user+".kh_dm_02 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_02 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_02 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_02 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_02 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh02(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c17"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			if (r1!=null)
			{
				...
				if (dk.Text=="C08<=C05") dk.Text="C08<=C01";
				if (dk.Text=="C12<=C05") dk.Text="C12>=C05";
				if (dk.Text=="C05<=C01") dk.Text="C05<=C01 AND C05<=C12";
			}
			else
			{
				ten.Text="";
			...
			try
			{
				...
				//if (decimal.Parse(dataGrid1[i_row,7].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,14].ToString())>decimal.Parse(dataGrid1[i_row,7].ToString()) || decimal.Parse(dataGrid1[i_row,19].ToString())>decimal.Parse(dataGrid1[i_row,7].ToString()) || decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()))
				if (decimal.Parse(dataGrid1[i_row,7].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,14].ToString())<decimal.Parse(dataGrid1[i_row,7].ToString()) || decimal.Parse(dataGrid1[i_row,19].ToString())>decimal.Parse(dataGrid1[i_row,7].ToString()) || decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()))
				{
					MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
					dataGrid1[i_row,i_col]=0;
				}
				if (i_col>=4 && i_col<=5 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString());
					if (tc>decimal.Parse(dataGrid1[i_row,3].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}
				}
				if (i_col>=8 && i_col<=9 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,8].ToString())+decimal.Parse(dataGrid1[i_row,9].ToString());
					if (tc>decimal.Parse(dataGrid1[i_row,7].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}
				}
				if (i_col>=11 && i_col<=13 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,11].ToString())+decimal.Parse(dataGrid1[i_row,12].ToString())+decimal.Parse(dataGrid1[i_row,13].ToString());
					if (tc>decimal.Parse(dataGrid1[i_row,10].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}
				}
				if (i_col>=15 && i_col<=18 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,15].ToString())+decimal.Parse(dataGrid1[i_row,16].ToString())+decimal.Parse(dataGrid1[i_row,17].ToString())+decimal.Parse(dataGrid1[i_row,18].ToString());
					if (tc>decimal.Parse(dataGrid1[i_row,14].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}

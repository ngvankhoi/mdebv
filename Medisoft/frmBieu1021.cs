...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu1021 : System.Windows.Forms.Form
	{
		...
		public frmBieu1021(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu1021( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu1021));
            ...
            this.Name = "frmBieu1021";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "HĐTC :  Chi tiết về thu viện phí - bảo hiểm y tế";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu1021_Load);
		...
		private void frmBieu1021_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_1021 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from  "+m.user+".bieu_1021 a,"+m.user+".dm_1021 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_1021 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from  "+m.user+".bieu_1021 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from  "+m.user+".bieu_1021 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string exp="",stt="";
				if (i_row>=1 || dataGrid1[i_row,1].ToString()=="A")
				{
					exp="ma>=2";
					stt="A";
				}
		...
		{
			string exp="ma>=2",stt="A";
			DataRow[] r;
			Int16 iRec;
			decimal l_tong;
			// for(int k=3;k<ds.Tables[0].Columns.Count;k++) goc truongthuy 17052014
            for (int k = 3; k <= 4; k++)
			{

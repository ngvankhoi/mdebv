...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu031 : System.Windows.Forms.Form
	{
		...
		public frmBieu031(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu031( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu031));
            ...
            this.Name = "frmBieu031";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Hoạt động điều trị";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu031_Load);
		...
		private void frmBieu031_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_031_dk").Tables[0];
			load_grid(false);
		...
		private void load_grid(bool dm)
		{
			if (dm)
			{
				ds=m.get_data("select * from "+m.user+".dm_031 where ma<3 order by ma");
				foreach(DataRow r in m.get_data("select * from "+m.user+".btdkp_bv where loai=0 order by makp").Tables[0].Rows)
					m.updrec_bieu(ds.Tables[0],int.Parse(r["makp"].ToString())+2,r["tenkp"].ToString(),11);
			}
			else load_data();
...
//			if (bPhatsinh)
//			{
//				sql="select a.*,b.stt,b.ten from "+m.user+".bieu_031 a,"+m.user+".dm_031 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
//				ds=m.get_data(sql);
//			}
//			else
//			{
...
//			}
		...
		private void load_value(bool load)
		{
			load_grid(true);
			DataRow[] dr;
			DataTable dt1=m.get_data("select * from "+m.user+".bieu_031 where id="+l_id).Tables[0];
			if (dt1.Rows.Count==0) ds.Clear();
			else
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_031 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butMoi_Click(object sender, System.EventArgs e)
		{
			...
			foreach(DataRow r in m.get_data("select makp,kehoach from "+m.user+".btdkp_bv order by makpbo").Tables[0].Rows)
			{
				...
				if (dr!=null) dr["c01"]=r["kehoach"].ToString();
			}
			ena_object(true);
			ngay.Focus();
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_031 where id="+l_id);
					...
					{
						m.upd_bieu031(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c10"].ToString()),int.Parse(r[i]["c11"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		{
			Color c = Color.Black;
            if (this.dataGrid1[row,1].ToString().Trim()=="1")
			{
				c=Color.Red;
				i_tong=row;
			}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string exp="ma>2",stt="1",s="060709101112",s1="05060709101112";
				if (s1.IndexOf(i_col.ToString().PadLeft(2,'0'))==-1)
				{
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
				...
				if (s.IndexOf(i_col.ToString().PadLeft(2,'0'))!=-1 && dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,i_col].ToString())>decimal.Parse(dataGrid1[i_row,5].ToString()) || decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,6].ToString()) || decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,9].ToString()) || decimal.Parse(dataGrid1[i_row,11].ToString())>decimal.Parse(dataGrid1[i_row,9].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}
		...
		{
			string exp="ma>2",stt="1",s1="05060709101112";
			DataRow[] r;
			Int16 iRec;
			decimal l_tong;
			for(int k=3;k<ds.Tables[0].Columns.Count;k++)
			{
				if (s1.IndexOf(k.ToString().PadLeft(2,'0'))==-1)
				{

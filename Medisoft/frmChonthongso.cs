...
namespace Medisoft
{
	...
	public class frmChonthongso : System.Windows.Forms.Form
	{
		...
		private int i_loai,i_benhnhan,i_thuoc,iUserID, i_idchinhanh=0;
		private string user,sql,kho_phieu;
        public int i_makp, i_nhom, i_phieu, i_macstt, i_buoi, i_somay, i_khudt=0;
        ...
        public decimal d_idxuattutruc = 0;

		/// <summary>
		/// Required designer variable.
		/// </summary>

		public frmChonthongso(LibMedi.AccessData acc,int thuoc,int loai,int benhnhan,string makp,bool thua,string _nhomkho, int _khudt,int i_UserID)
		{
			InitializeComponent();
            ...
            this.iUserID = i_UserID;
		}
        public decimal IDXuatTuTruc
        {
            set
            {
                d_idxuattutruc = value;
            }
        }
        /// <summary>
        ...
        public frmChonthongso(LibMedi.AccessData acc, int thuoc, int loai, int benhnhan, string makp, bool thua, string _nhomkho,int i_UserID)
        {
            InitializeComponent();
            lan.Read_Language_to_Xml(this.Name.ToString(), this);
            lan.Changelanguage_to_English(this.Name.ToString(), this);
		...
		private void load_makp()
		{
			if (nhom.SelectedIndex!=-1)
			{
                ...
                if (i_idchinhanh > 0) sql += " and a.chinhanh=" + i_idchinhanh;
				sql+=" order by a.stt";
		...
		{
            if (this.ActiveControl == makp || sender == null)
			{
				if (i_loai==2) load_matutruc();
		...
		private void butOk_Click(object sender, System.EventArgs e)
		{
            ...
            {//linh 16102012
                ...
                if (i_loai != 2)
                {
                    d.upd_duyet(s_mmyy, l_duyet, i_nhom, s_ngay, i_loai, i_phieu, i_makp, 0, this.iUserID, i_makp, 0);//gam 09/09/2011
                }
                else//tu truc: makp la matu truc, makhoa: m\la makp cua bn dang dieu tri
                {
                    d.upd_duyet(s_mmyy, l_duyet, i_nhom, s_ngay, i_loai, i_phieu, i_macstt, 0, this.iUserID, i_makp, 0);//gam 09/09/2011
                }
                //d.upd_duyet(s_mmyy, l_duyet, i_nhom, s_ngay, i_loai, i_phieu, i_makp, 0, this.iUserID, i_makp, 0);//gam 09/09/2011
                foreach (DataRow rkho in m.get_data("select * from " + user + ".d_dmkho where hide=0 and id in (" + s_makho.Substring(0, s_makho.Length - 1) + ")").Tables[0].Rows)
                    d.upd_duyetkho(s_mmyy, l_duyet, int.Parse(rkho["id"].ToString()), 0);
            }
		...
		{
            if (this.ActiveControl == nhom || sender==null )
            {
                load_makp();
                load_makho();
            }

...
namespace Medisoft
{
	...
	/// </summary>
	public class frmTkketqua : System.Windows.Forms.Form
	{
        ...
        public frmTkketqua(AccessData acc)
		{
			//
		...
		private void frmTkketqua_Load(object sender, System.EventArgs e)
		{
		...
		private void butTim_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                if (dotuoi.SelectedIndex != -1)
                {
                    ...
                    if (s_dotuoi.IndexOf(">") != -1)//binh
                    {
                        //sql += " and to_number(to_char(now(),'yyyy'),'0000')-to_number(a.namsinh,'0000')>" + int.Parse(dotuoi.Substring(1));
                        sql += " and to_number(to_char(a.ngay,'yyyy'),'0000')-to_number(b.namsinh,'0000')>" + int.Parse(s_dotuoi.Substring(1));
                    }
                    else
                    {
                        int i1 = int.Parse(s_dotuoi.Substring(0, s_dotuoi.IndexOf("-")));
                        int i2 = int.Parse(s_dotuoi.Substring(s_dotuoi.IndexOf("-") + 1));
                        sql += " and to_number(to_char(a.ngay,'yyyy'),'0000')-to_number(b.namsinh,'0000') between " + i1 + " and " + i2; //sql += " and to_number(to_char(now(),'yyyy'),'0000')-to_number(a.namsinh,'0000') between " + i1 + " and " + i2;
                    }
                }
				...
				dllReportM.frmReport f=new dllReportM.frmReport(m,ds,solieu.Text.ToUpper()+" "+tuden,"rptTkketqua.rpt");
				f.ShowDialog();
			}
		...
		private void frmTkketqua_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{

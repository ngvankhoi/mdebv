...
namespace Medisoft
{
	...
	/// </summary>
	public class frmKh132 : System.Windows.Forms.Form
	{
		...
		public frmKh132(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh132( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh132));
            ...
            this.butMoi.Location = new System.Drawing.Point(144, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(78, 25);
            ...
            this.butSua.Size = new System.Drawing.Size(78, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(300, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(378, 500);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(78, 25);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(534, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(456, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(78, 25);
            ...
            this.Text = "Biểu 13.2//Thực hiện công tác phòng bệnh";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh132_Load);
		...
		private void frmKh132_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_132_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_132 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_132 a,"+m.user+".kh_dm_132 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_132 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_132 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_132 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_132 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17+c18+c19+c20+c21+c22+c23+c24+c25+c26+c27>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh132(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c26"].ToString()),int.Parse(r[i]["c27"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (decimal.Parse(dataGrid1[i_row,6].ToString())>decimal.Parse(dataGrid1[i_row,5].ToString())	|| decimal.Parse(dataGrid1[i_row,8].ToString())>decimal.Parse(dataGrid1[i_row,7].ToString()) || decimal.Parse(dataGrid1[i_row,10].ToString())>decimal.Parse(dataGrid1[i_row,9].ToString()) || decimal.Parse(dataGrid1[i_row,12].ToString())>decimal.Parse(dataGrid1[i_row,11].ToString())	|| decimal.Parse(dataGrid1[i_row,14].ToString())<decimal.Parse(dataGrid1[i_row,13].ToString()) || decimal.Parse(dataGrid1[i_row,16].ToString())+decimal.Parse(dataGrid1[i_row,17].ToString())>decimal.Parse(dataGrid1[i_row,15].ToString()) || decimal.Parse(dataGrid1[i_row,28].ToString())+decimal.Parse(dataGrid1[i_row,29].ToString())>decimal.Parse(dataGrid1[i_row,27].ToString()))
				{

...
namespace Medisoft
{
	...
	public class frmKh08 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "Tình hình trang thiết bị y tế của địa phương";
		...
		private void frmKh08_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_08_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_08 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_08 a,"+m.user+".kh_dm_08 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_08 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_08 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_08 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_08 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06>0";
					...
					for(Int16 i=0;i<iRec;i++)
					{
                        ...
                        m.upd_kh08(l_id, int.Parse(r[i]["ma"].ToString()), ngay.Text, int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c05"].ToString()), r[i]["c06"].ToString(), i_userid);
                        //
					}
				}
				ena_object(false);
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Tên các trang thiết bị theo 4 khoa chính";
			TextCol1.Width = 300;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				//i_row=dataGrid1.CurrentCell.RowNumber;
				if (i_col<3) return;
				s=dataGrid1[i_row,1].ToString().Trim();
				if (s=="A" || (i_row>=1 && i_row<=6)) {stt="A";exp="ma>=2 and ma<=6";}
				else if (s=="B" || (i_row>=6 && i_row<=14)) {stt="B";exp="ma>=8 and ma<=14";}
				else if (s=="C" || (i_row>=14 && i_row<=26)) {stt="C";exp="ma>=16 and ma<=26";}
				else if (s=="D" || (i_row>=26 && i_row<=34)) {stt="D";exp="ma>=28 and ma<=34";}
				else if (s=="E" || (i_row>=34 && i_row<=52)) {stt="E";exp="ma>=36 and ma<=52";}
				else if (s=="F" || i_row>=52) {stt="F";exp="ma>=54";}
				if (stt!="")
				{
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
				...
				if (decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString())+decimal.Parse(dataGrid1[i_row,7].ToString())+decimal.Parse(dataGrid1[i_row,8].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()))
				{
		...
		private void Tongcong()
		{
			...
			{
				sum("ma>=2 and ma<=6","A",k);
				sum("ma>=8 and ma<=14","B",k);
				sum("ma>=16 and ma<=26","C",k);
				sum("ma>=28 and ma<=34","D",k);
				sum("ma>=36 and ma<=52","E",k);
				sum("ma>=54","F",k);
			}
		}

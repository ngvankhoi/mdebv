...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu05 : System.Windows.Forms.Form
	{
		...
		public frmBieu05(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu05( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu05));
            ...
            this.Text = "Hoạt động sức khỏe sinh sản";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu05_Load);
		...
		private void frmBieu05_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_05 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".bieu_05 a,"+m.user+".dm_05 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_05 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_05 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_05 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_05 where id="+l_id);
					string exp="soluong>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu05(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["soluong"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;

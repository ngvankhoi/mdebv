...
namespace Medisoft
{
	...
	public class frmChonchidinh : System.Windows.Forms.Form
	{
        ...
        private string s_idnhom = "", s_idloai = "";
		private DataColumn dc;
		private AccessData m;
		private CheckBox chkbox;
		private	TabPage tab;
		private ToolTip tooltip;
        private bool bDongia_lietke,bVpkhoa,bChidinh_thutienlien;
        private int i_madoituong, i_loai, v1, v2, vitri_jl;
		...
		public frmChonchidinh(AccessData acc,string mabn,int madoituong,string loaivp,string mucvp,int loai,string sothe,int vt1,int vt2,bool vpkhoa)
		{
			//
		...
		private void load()
		{
			...
			foreach(DataRow r in m.get_data(sql).Tables[0].Rows)
			{
				...
				foreach(DataRow r1 in m.get_data(sql).Tables[0].Rows)
				{
					...
					Addchkbox(r1["ten"].ToString(),r1["id"].ToString(),t,l,r1[fie].ToString(),r1[fie].ToString(),new EventHandler(chkEvent));
					t+=23;
					j++;
				}
				tab.AutoScroll=true;
		...
		public void Addchkbox(string text,string name,int t,int l,string gia_th,string gia_bh,EventHandler onClickEvent)
		{
            ...
            if (bDongia_lietke) tooltip.SetToolTip(chkbox,text+"\n"+((gia_th == gia_bh) ? gia_th : gia_th + "\n" + gia_bh));
            else tooltip.SetToolTip(chkbox,text);
		}
        ...
        private void f_Load_Tree()
        {
            try
            {
                ...
                sql = "select distinct a.* from " + m.user + ".v_nhomvp a, " + m.user + ".v_loaivp b where a.ma=b.id_nhom ";
                if (s_loaivp != "") sql += " and b.id in(" + s_loaivp.Trim().Trim(',') + ")";
                sql += "   order by a.stt";
                adsnhom = m.get_data(sql);// m_v.f_get_v_nhomvp_frmgiavp();
                sql = "select * from " + m.user + ".v_loaivp ";
                if (s_loaivp != "") sql += " where id in(" + s_loaivp.Trim().Trim(',') + ")";
                sql += "  order by id_nhom, stt";

...
namespace Medisoft
{
    public class frmGoiDichvu : Form
    {
        ...
        private bool bBienlaichiuthue;
        private bool bChenhlech_doituong;
        private bool bChidinh_exp_txt;
        private bool bChidinh_loaivp,bTiemchung=false;
        private bool bChidinh_thutienlien;
        private bool bChuky;
        private string Bienlaichiuthue_gia;
        private bool bLocdichvu;
        private bool bLocthan;
        private bool bNew;
        private bool bUpdate_chidinh;
        ...
        public frmGoiDichvu(int nhom, int _makp, string _ngay, int _loai, int _phieu, string _mmyy, string _makho, string _manguon, decimal duyet,
            string mabn, decimal mavaovien, decimal maql, decimal idkhoa, int userid, decimal idduoc, int _makhoa, string _ngayvv, string _mapt, string _mamau,
            bool _bNew, decimal idlocthan, int _madoituong, string _matrai, decimal _idchidinh, string _sothe, string _maicd, string _chandoan, string _mabs,
            int _loaiba, bool update_chidinh, bool locthan, decimal _idchaythan,int _tiemchung)
        {
            this.InitializeComponent();
            this.user = this.d.user;
            m_ds_danhsach = m.get_data("select 0 as id, 0 as idduoc, '' as ten from dual").Clone();
        ...
        private void AddGridTableStyle1()
        {
            try
            {
                ...
                this.ts.GridColumnStyles.Add(new DataGridComboBoxColumn(this.dtnguon, 1, 1));
        ...
        private void butIn_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    DataTable ta = this.ds.Tables[0].Copy();
            //    string str = "Nam";
            //    string str2 = "";
            //    string str3 = "";
            //    try
            //    {
            //        foreach (DataRow row in this.m.get_data("select hoten,to_number(to_char(sysdate,'yyyy'),'9999')-to_number(namsinh,'9999') as tuoi,phai from " + this.user + ".btdbn where mabn='" + this.s_mabn + "'").Tables[0].Rows)
            //        {
            //            str = (row["phai"].ToString() == "0") ? "Nam" : "Nữ";
            //            str2 = row["tuoi"].ToString();
            //            str3 = row["hoten"].ToString();
            //        }
            //    }
            //    catch
            //    {
            //    }
            //    ta.Columns.Add("cachdung");
            //    int count = ta.Rows.Count;
            //    int num2 = 0x15;
            //    if (count < num2)
            //    {
            //        for (int i = count; i < (num2 - count); i++)
            //        {
            ...
            //        }
            //    }
            //    new frmReport(this.m, ta, "ylenh_cstt.rpt", this.s_mabn, str3, "", "", this.ngay.Substring(0, 10), "", str2, str, "", "").ShowDialog();
            //}
        }
        ...
        private void butLuu_Click(object sender, EventArgs e)
        {
           ...
           // dtct.AcceptChanges();
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    if (d.get_tutrucct_dutru(mmyy, makp, int.Parse(makho.Trim(',')), 1, int.Parse(rbd["mabd"].ToString()), 0, i_nhom).Rows.Count == 0)
                    {
                ...
                if (l_duyet == 0)
                {
                    ...
                    d.upd_duyet(mmyy, l_duyet, i_nhom, ngay, loai, phieu, makp, 0, i_userid, makhoa,0);
                    foreach (DataRow r1 in m.get_data("select * from " + user + ".d_dmkho where hide=0 and id in (" + makho.Trim(',') + ")").Tables[0].Rows)
                        d.upd_duyetkho(mmyy, l_duyet, int.Parse(r1["id"].ToString()), 0);
                }
                else d.upd_duyet(mmyy, l_duyet, i_nhom, ngay, loai, phieu, makp, 0, i_userid, makhoa,0);
                ...
                foreach (DataRow r1 in ds.Tables[0].Rows)
                {
                    ...
                    d.upd_xtutrucct(mmyy, l_id, stt,
                        (int.Parse(r1["madoituong"].ToString()) == 0) ? 5 : int.Parse(r1["madoituong"].ToString()), int.Parse(makho.Trim(',')),//int.Parse(r1["makho"].ToString()) 
                ...
                foreach (DataRow r1 in ds.Tables[0].Select("soluong>0"))
                {
                    ...
                    v.upd_vpkhoa(l_idvpkhoa, s_mabn, l_mavaovien, l_maql, l_idkhoa, ngay, matrai, int.Parse(r1["madoituong"].ToString()), int.Parse(r1["mabd"].ToString()), decimal.Parse(r1["soluong"].ToString()), gia, 0, i_userid, 0);
                    d.upd_theodoicongno(d.insert, s_mabn, l_mavaovien, l_maql, l_idkhoa, int.Parse(r1["madoituong"].ToString()), Math.Round(decimal.Parse(r1["soluong"].ToString()) * gia, 3), "vienphi");
            ...
            //    sql += " order by a.stt";
                    ...
                    upd_data(int.Parse(str), 1, _madt, 0, 0);
            //}
        ...
        private void upd_detail(bool chenhlech, int i_mavp, decimal d_soluong, decimal dongia, int idtrongoi)
        {
            ...
            if (r != null)
            {
                ...
                if (!v.upd_chidinh(l_idcd, s_mabn, l_mavaovien, l_maql, l_idkhoa, ngay, loai, matrai, madoituong, i_mavp, d_soluong, (dongia != 0) ? dongia : d_dongia, (dongia != 0) ? 0 : d_vattu, i_userid, 1, 1, l_idchidinh, (maicd == "" && chandoan != "") ? "U00" : maicd, chandoan, mabs, loaiba, ""))
                {
                ...
                if (dg_giam != 0)
                {
                    ...
                    v.upd_chidinh(l_id, s_mabn, l_mavaovien, l_maql, l_idkhoa, ngay, loai, matrai, cl_doituong, i_mavp, d_soluong, dg_giam, vt_giam, i_userid, 1, 1, l_idchidinh, (maicd == "" && chandoan != "") ? "U00" : maicd, chandoan, mabs, loaiba, "");
                ...
                if (bNew)
                {
                    ...
                    {
                        l_idcd = v.get_id_chidinh;
                        if (!v.upd_chidinh(l_idcd, s_mabn, l_mavaovien, l_maql, l_idkhoa, ngay, loai, matrai, int.Parse(dr1["madoituong"].ToString()), int.Parse(dr1["id_gia"].ToString()), (int)decimal.Parse(dr1["soluong"].ToString()), 0, 0, i_userid, 1, 1, 0, (maicd == "" && chandoan != "") ? "U00" : maicd, chandoan, mabs, loaiba, ""))
                        {
                            MessageBox.Show(lan.Change_language_MessageText("Không cập nhật được thông tin chỉ định !"), LibMedi.AccessData.Msg);
                            return;
                        }
        ...
        private void load_treeview()
        {
            ...
            try
            {
                ...
                sql = "select a.id,a.ten,a.id_loai,b.ten tenloai,b.id_nhom,c.ten tennhom from " + m.user + ".v_giavp a," + m.user + ".v_loaivp b," + m.user + ".v_nhomvp c where a.trongoi=1 and a.loaitrongoi=1 and a.id_loai=b.id and b.id_nhom=c.ma order by b.id_nhom,b.id,a.ten";
        ...
        private void prn(string tenfile, bool chitiet)
        {
            ...
            if (dset.Tables[0].Rows.Count > 0)
            {
                ...
                new frmReport(this.m, dset, "", tenfile, true).ShowDialog();
            }
            else
            {
        ...
        private void upd_detail(bool chenhlech, int i_mavp, decimal d_soluong, decimal dongia, int idtrongoi, int madt)
        {
            ...
            if (row != null)
            {
                ...
                if (!this.v.upd_chidinh(this.idchidinh, this.s_mabn, this.l_mavaovien, this.l_maql, this.l_idkhoa, this.get_ngaygio(this.ngay), this.v.iNoitru, this.matrai, madt, i_mavp, d_soluong, (dongia != 0M) ? dongia : ddongia, (dongia != 0M) ? 0M : dvtth, this.i_userid, 1, 1, (this.l_idchidinh == 0L) ? this.idchidinh : this.l_idchidinh, this.maicd, this.chandoan, this.mabs, this.loaiba, ""))
                {
                    MessageBox.Show(this.lan.Change_language_MessageText("Không cập nhật được thông tin chỉ định !"), "Medisoft");
                }
                else
                {
                    ...
                    {
                        this.idchidinh = this.v.get_id_chidinh;
                        this.v.upd_chidinh( this.idchidinh, this.s_mabn, this.l_mavaovien, this.l_maql, this.l_idkhoa, this.get_ngaygio(this.ngay), this.v.iNoitru, this.matrai, this.cl_doituong, i_mavp, d_soluong, num2, num3, this.i_userid, 0, 0, this.l_idchidinh, this.maicd, this.chandoan, this.mabs, this.loaiba, "");
                    ...
                    if (bNew)
                    {
                        ...
                        foreach (DataRow dr1 in dttam.Rows)
                        {
                            ...
                            if (!v.upd_chidinh(idchidinh, s_mabn, l_mavaovien, l_maql, l_idkhoa, ngay, this.v.iNoitru, matrai, int.Parse(dr1["madoituong"].ToString()), int.Parse(dr1["id_gia"].ToString()), (int)decimal.Parse(dr1["soluong"].ToString()), 0, 0, i_userid, 1, 1, 0, (maicd == "" && chandoan != "") ? "U00" : maicd, chandoan, mabs, loaiba, ""))
                            {
                                //MessageBox.Show(lan.Change_language_MessageText("Không cập nhật được thông tin chỉ định !"), LibMedi.AccessData.Msg);
                                return;
                            }
        ...
        private void butHuy_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(lan.Change_language_MessageText("Đồng ý hủy '" + treeLan.SelectedNode.Text + "' ?"), LibMedi.AccessData.Msg, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                DataTable tmp1 = m.get_data("select * from " + user + mmyy + ".v_chidinh where id=" + l_id).Tables[0];
                m.execute_data("delete from " + user + mmyy + ".v_chidinh where id=" + l_id);
                m.execute_data("delete from " + user + mmyy + ".v_chidinh where id_goi=" + l_id);
                //if (tmp.Rows.Count > 0)
                //{

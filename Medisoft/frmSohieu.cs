...
namespace Medisoft
{
	...
	public class frmSohieu : System.Windows.Forms.Form
	{
		...
		public int iLoai,iKyhieu,iAll=0;
		...
		public frmSohieu(AccessData acc)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            this.butOK.Location = new System.Drawing.Point(35, 121);
		...
		private void frmSohieu_Load(object sender, System.EventArgs e)
		{
            ...
            c196.SelectedValue = m.iUservp;

			sohieu.DisplayMember="SOHIEU";
			sohieu.ValueMember="ID";
			load_sohieu();
			sohieu.SelectedValue=ds.Tables[0].Rows[0]["sohieu"].ToString();
		}

		private void load_sohieu()
		{
			sql="select * from "+user+".v_quyenso where khambenh=1 and loai like '%"+loai.SelectedValue.ToString()+"%'";
			if (!checkBox1.Checked) sql+=" and soghi<den";
            if (m.bKyhieu_may) sql += " and computerid=" + m.iRownum;
            else if (!checkBox1.Checked && c196.SelectedIndex!=-1) sql += " and userid=" + decimal.Parse(c196.SelectedValue.ToString());
        ...
        private bool bMahoamatkhau
        {
            get
            {
                return m.get_data("select ten from " + user + ".v_thongso where id=-13").Tables[0].Rows.Count > 0;
            }
        }
		...
		private void butOK_Click(object sender, System.EventArgs e)
		{
            ...
            if (bMahoamatkhau)
            {
                string smakhau = matkhau.Text.ToString().Trim().ToUpper();
                sql = "select * from " + user + ".v_dlogin where id=" + int.Parse(c196.SelectedValue.ToString());
                sql += " and password_='" + m.encode(matkhau.Text);
			...
			try
			{
				ds.Tables[0].Rows[0]["loai"]=iLoai.ToString();
				ds.Tables[0].Rows[0]["sohieu"]=iKyhieu.ToString();
				ds.WriteXml("..//..//..//xml//v_thongso.xml");
			}
			catch{}
            m.writeXml("thongso", "c196", c196.SelectedValue.ToString());
			m.close();this.Close();
		}

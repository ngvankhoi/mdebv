...
namespace Medisoft
{
	...
	/// </summary>
	public class frmKh03 : System.Windows.Forms.Form
	{
		...
		public frmKh03(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh03(string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh03));
            ...
            this.butSua.Location = new System.Drawing.Point(277, 521);
            ...
            this.butLuu.Location = new System.Drawing.Point(347, 521);
            ...
            this.butBoqua.Location = new System.Drawing.Point(417, 521);
            ...
            this.butHuy.Location = new System.Drawing.Point(487, 521);
		...
		private void frmKh03_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_03_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_03 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_03 a,"+m.user+".kh_dm_03 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_03 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_03 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_03 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_03 where id="+l_id);
					...
					{
						m.upd_kh03(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=4 && i_col<=9 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString())+decimal.Parse(dataGrid1[i_row,7].ToString())+decimal.Parse(dataGrid1[i_row,8].ToString())+decimal.Parse(dataGrid1[i_row,9].ToString());
				...
				if (i_col>=11 && i_col<=22 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,11].ToString())+decimal.Parse(dataGrid1[i_row,12].ToString())+decimal.Parse(dataGrid1[i_row,13].ToString())+decimal.Parse(dataGrid1[i_row,14].ToString())+decimal.Parse(dataGrid1[i_row,15].ToString())+decimal.Parse(dataGrid1[i_row,16].ToString())+decimal.Parse(dataGrid1[i_row,17].ToString())+decimal.Parse(dataGrid1[i_row,18].ToString())+decimal.Parse(dataGrid1[i_row,19].ToString())+decimal.Parse(dataGrid1[i_row,20].ToString())+decimal.Parse(dataGrid1[i_row,21].ToString())+decimal.Parse(dataGrid1[i_row,22].ToString());

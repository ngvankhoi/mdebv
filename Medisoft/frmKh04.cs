...
namespace Medisoft
{
	...
	public class frmKh04 : System.Windows.Forms.Form
	{
		...
		private void frmKh04_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_04_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_04 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_04 a,"+m.user+".kh_dm_04 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_04 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_04 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_04 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_04 where id="+l_id);
					...
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh04(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c17"].ToString()),r[i]["c18"].ToString(),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=4 && i_col<=13 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString())+decimal.Parse(dataGrid1[i_row,7].ToString())+decimal.Parse(dataGrid1[i_row,8].ToString())+decimal.Parse(dataGrid1[i_row,9].ToString())+decimal.Parse(dataGrid1[i_row,10].ToString())+decimal.Parse(dataGrid1[i_row,11].ToString())+decimal.Parse(dataGrid1[i_row,12].ToString())+decimal.Parse(dataGrid1[i_row,13].ToString());
				...
				if (i_col>=15 && i_col<=19 && dataGrid1[i_row,i_col].ToString()!="")
				{
					tc=decimal.Parse(dataGrid1[i_row,15].ToString())+decimal.Parse(dataGrid1[i_row,16].ToString())+decimal.Parse(dataGrid1[i_row,17].ToString())+decimal.Parse(dataGrid1[i_row,18].ToString())+decimal.Parse(dataGrid1[i_row,19].ToString());

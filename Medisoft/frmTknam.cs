...
namespace Medisoft
{
	...
	/// </summary>
	public class frmTknam : System.Windows.Forms.Form
	{
        ...
        public frmTknam(AccessData acc)
		{
			//
		...
		private void InitializeComponent()
		{
            ...
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmTknam));
            ...
            this.Text = "So sánh tình hình mắc bệnh theo năm";
            this.Load += new System.EventHandler(this.frmTknam_Load);
            this.MouseMove += new System.Windows.Forms.MouseEventHandler(this.frmTkravien_MouseMove);
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmTknam_KeyDown);
		...
		private void frmTknam_Load(object sender, System.EventArgs e)
		{
            m_user = m.user;
			dt=m.get_data("select cicd10,vviet from "+ m_user +".icd10 order by cicd10").Tables[0];
			dsxml.ReadXml("..//..//..//xml//t_icd10.xml");
			dsdt.ReadXml("..//..//..//xml//dotuoi.xml");
			dssl.ReadXml("..//..//..//xml//t_nam.xml");
            ...
            ttlucrv.DataSource = m.get_data("select * from " + m_user + ".ttxk order by ma").Tables[0];
            ...
            mann.DataSource = m.get_data("select * from " + m_user + ".btdnn_bv order by mann").Tables[0];

            madantoc.DataSource = m.get_data("select * from " + m_user + ".btddt order by madantoc").Tables[0];
			madantoc.DisplayMember="DANTOC";
			madantoc.ValueMember="MADANTOC";

            matt.DataSource = m.get_data("select * from " + m_user + ".btdtt order by matt").Tables[0];
            ...
            tu.Text = m.ngayhienhanh_server.Substring(6,4);
            den.Text = tu.Text;
		}
		...
		private void load_quan()
		{
			try
			{
                maqu.DataSource = m.get_data("select * from " + m_user + ".btdquan where matt='" + matt.SelectedValue.ToString() + "'" + " order by maqu").Tables[0];
			}
			catch{}
		}
		...
		private void load_pxa()
		{
			try
			{
				maphuongxa.DataSource=m.get_data("select * from "+ m_user +".btdpxa where maqu='"+maqu.SelectedValue.ToString()+"'"+" order by maphuongxa").Tables[0];
			}
			catch{}
		}
		...
		private void tu_Validated(object sender, System.EventArgs e)
		{
			...
			tu.Text=tu.Text.Trim().PadLeft(4,'0');
		...
		private void den_Validated(object sender, System.EventArgs e)
		{
			...
			den.Text=den.Text.Trim().PadLeft(4,'0');
			if (tu.Text=="")
			{
				tu.Focus();
				return;
			}
			if (int.Parse(tu.Text)>int.Parse(den.Text))
			{
				MessageBox.Show(lan.Change_language_MessageText("Đến năm không được nhỏ hơn từ năm !"));
				den.Focus();
				return;
			}
		}
		...
		private void butTim_Click(object sender, System.EventArgs e)
		{
			try
			{
                ...
                if (solieu.SelectedIndex != 4)
                {
                    ...
                    if (maicd.Text != "") sql += " and ac.maicd='" + maicd.Text + "'";
                    if (checkBox1.Checked) sql += " and ac.maicd in (select ma from " + m.user + ".dmicd10)";
                }
                else
                {
                    if (tu.Text != "" && den.Text != "")
				...
				foreach(DataRow r1 in ds.Tables[0].Rows)
				{
					...
					if (r2!=null) r1["ten"]=r1["maicd"].ToString().PadRight(9,' ')+" "+r2["vviet"].ToString();
					else r1["ten"]="xxx";
				}
		...
		private void frmTknam_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{

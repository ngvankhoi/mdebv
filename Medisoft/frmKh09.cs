...
namespace Medisoft
{
	...
	public class frmKh09 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "Chăm sóc sức khoẻ trẻ em";
		...
		private void frmKh09_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_09_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_09 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_09 a,"+m.user+".kh_dm_09 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_09 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_09 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_09 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_09 where id="+l_id);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=13 && i_col<=31 && dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,14].ToString())>decimal.Parse(dataGrid1[i_row,13].ToString()) || decimal.Parse(dataGrid1[i_row,16].ToString())>decimal.Parse(dataGrid1[i_row,15].ToString()) || decimal.Parse(dataGrid1[i_row,18].ToString())>decimal.Parse(dataGrid1[i_row,17].ToString()) || decimal.Parse(dataGrid1[i_row,20].ToString())>decimal.Parse(dataGrid1[i_row,19].ToString()) || decimal.Parse(dataGrid1[i_row,22].ToString())>decimal.Parse(dataGrid1[i_row,21].ToString()) || decimal.Parse(dataGrid1[i_row,24].ToString())>decimal.Parse(dataGrid1[i_row,23].ToString()) || decimal.Parse(dataGrid1[i_row,26].ToString())>decimal.Parse(dataGrid1[i_row,25].ToString()) || decimal.Parse(dataGrid1[i_row,30].ToString())+decimal.Parse(dataGrid1[i_row,31].ToString())>decimal.Parse(dataGrid1[i_row,29].ToString()))
					{

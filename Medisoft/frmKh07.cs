...
namespace Medisoft
{
	...
	public class frmKh07 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "Tình hình sản xuất kinh doanh dược của huyện";
		...
		private void frmKh07_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_07_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_07 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_07 a,"+m.user+".kh_dm_07 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_07 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_07 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_07 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_07 where id="+l_id);
					...
					{//ThanhCuong 25/02/2012
                        m.upd_kh07(l_id, int.Parse(r[i]["ma"].ToString()), ngay.Text, int.Parse(r[i]["c01"].ToString()),
                            ...
                            int.Parse(r[i]["c08"].ToString()), int.Parse(r[i]["c09"].ToString()), i_userid, r[i]["c10"].ToString());
                        //
					}
				}
				ena_object(false);
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Cơ sở sản xuất kinh doanh(Hiệu thuốc,Đại lý... huyện)";
			TextCol1.Width = 286;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=6 && i_col<=11 && dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,7].ToString())>decimal.Parse(dataGrid1[i_row,6].ToString()) || decimal.Parse(dataGrid1[i_row,9].ToString())>decimal.Parse(dataGrid1[i_row,8].ToString()) || decimal.Parse(dataGrid1[i_row,11].ToString())>decimal.Parse(dataGrid1[i_row,10].ToString()))
					{

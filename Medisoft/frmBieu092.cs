...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu092 : System.Windows.Forms.Form
	{
		...
		public frmBieu092(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu092( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu092));
            ...
            this.Name = "frmBieu092";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Hoạt động nghiên cứu khoa học";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu092_Load);
		...
		private void frmBieu092_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_092 order by ma");
			else load_data();
...
//			if (bPhatsinh)
//			{
				sql="select * from "+m.user+".bieu_092 where id="+l_id+" order by ma";
				ds=m.get_data(sql);
//			}
//			else
//			{
...
//			}
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_092 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_092 where id="+l_id).Tables[0].Rows)
			{
				dr=ds.Tables[0].Select("ma="+int.Parse(r["ma"].ToString()));
				if (dr!=null)
				{
					dr[0]["ten"]=r["ten"].ToString();
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_092 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_092 where id="+l_id);
					string exp="c01+c02+c03+c04>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu092(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,r[i]["ten"].ToString(),Decimal.Parse(r[i]["c01"].ToString()),
							int.Parse(r[i]["c02"].ToString()),int.Parse(r[i]["c03"].ToString()),int.Parse(r[i]["c04"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
...
//			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				ten.Text="";
				if (i_col>2) ten.Text="<SỐ ĐỀ TÀI ĐÃ NGHIỆM THU>";
				else if (i_col==2) ten.Text="<KINH PHÍ (1000đ)>";
				if (i_col<2) return;

...
namespace Medisoft
{
	...
	/// </summary>
	public class frmKh145 : System.Windows.Forms.Form
	{
		...
		public frmKh145(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh145( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh145));
            ...
            this.butMoi.Location = new System.Drawing.Point(84, 500);
            this.butMoi.Name = "butMoi";
            this.butMoi.Size = new System.Drawing.Size(102, 25);
            ...
            this.butSua.Location = new System.Drawing.Point(186, 500);
            this.butSua.Name = "butSua";
            this.butSua.Size = new System.Drawing.Size(102, 25);
            ...
            this.butLuu.Location = new System.Drawing.Point(288, 500);
            this.butLuu.Name = "butLuu";
            this.butLuu.Size = new System.Drawing.Size(102, 25);
            ...
            this.butBoqua.Location = new System.Drawing.Point(390, 500);
            this.butBoqua.Name = "butBoqua";
            this.butBoqua.Size = new System.Drawing.Size(102, 25);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(594, 500);
            ...
            this.dataGrid1.Size = new System.Drawing.Size(776, 487);
            ...
            this.butHuy.Location = new System.Drawing.Point(492, 500);
            this.butHuy.Name = "butHuy";
            this.butHuy.Size = new System.Drawing.Size(102, 25);
            ...
            this.dataGrid2.Location = new System.Drawing.Point(8, 536);
            ...
            this.dataGrid2.Size = new System.Drawing.Size(776, 125);
            ...
            this.Text = "Biểu 14.5// Báo cáo thống kê tai nạn, thương tích";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh145_Load);
		...
		private void frmKh145_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_1451_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_1451 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_1451 a,"+m.user+".kh_dm_1451 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_1451 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_1451 where id="+l_id).Tables[0].Rows)
			{
		...
		private void load_grid1(bool dm)
		{
			if (dm)	ds1=m.get_data("select * from "+m.user+".kh_dm_1452 order by ma");
			else load_data1();
		...
		private void load_data1()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten,b.ten1,b.ten2,b.ten3 from "+m.user+".kh_bieu_1452 a,"+m.user+".kh_dm_1452 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds1=m.get_data(sql);
			}
			else
			{
		...
		private void load_value1(bool load)
		{
			ds1=m.get_data("select * from "+m.user+".kh_dm_1452 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_1452 where id="+l_id).Tables[0].Rows)
			{
				dr=ds1.Tables[0].Select("ma="+int.Parse(r["ma"].ToString()));
				if (dr!=null)
				{
					dr[0]["c01"]=r["c01"].ToString();
					dr[0]["c02"]=r["c02"].ToString();
			...
			if (load) dataGrid2.DataSource=ds1.Tables[0];
		}

		private void load_ngay()
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_1451 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_1451 where id="+l_id);
				...
				if (ds1.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_1452 where id="+l_id);
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Nội dung tổng hợp và phân loại tai nạn,thuơng tích";
			TextCol1.Width = 328;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				//i_row=dataGrid1.CurrentCell.RowNumber;
				if (i_col<3) return;
				if ((i_row>=0 && i_row<=9) || (dataGrid1[i_row,1].ToString()=="B") || (dataGrid1[i_row,1].ToString()=="A"))
				{
					exp="ma>=3 and ma<=9";
					stt=2;
				}
				else if ((i_row>=9 && i_row<=17) || (dataGrid1[i_row,1].ToString()=="C"))
				{
					exp="ma>=11 and ma<=17";
					stt=10;
				}
				else if ((i_row>=17 && i_row<=22) || (dataGrid1[i_row,1].ToString()=="D"))
				{
					exp="ma>=19 and ma<=22";
					stt=18;
				}
				else if ((i_row>=29 && i_row<=36) || (dataGrid1[i_row,1].ToString()=="5"))
				{
					exp="ma>=31 and ma<=36";
					stt=30;
				}
				else if ((i_row>=22 && i_row<=39) || (dataGrid1[i_row,1].ToString()=="E"))
				{
					exp="ma in (24,25,26,27,28,29,30,37,38,39)";
					stt=23;
				}
				else if ((i_row>=39 && i_row<=48) || (dataGrid1[i_row,1].ToString()=="F"))
				{
					exp="ma>=41 and ma<=48";
					stt=40;
				}
				if (exp!="")
				{
					...
					m.updrec_145(ds.Tables[0],stt,i_col,l_tong);
					if (stt==2) m.updrec_145(ds.Tables[0],1,i_col,l_tong);
					else if (stt==28)
					{
						exp="ma in (24,25,26,27,28,29,30,37,38,39)";
						stt=23;
		...
		private void Tongcong()
		{
			...
			{
				sum("ma>=3 and ma<=9",1,k);
				sum("ma>=3 and ma<=9",2,k);
				sum("ma>=11 and ma<=17",10,k);
				sum("ma>=19 and ma<=22",18,k);
				sum("ma>=31 and ma<=36",30,k);
				sum("ma in (24,25,26,27,28,29,30,37,38,39)",23,k);
				sum("ma>=41 and ma<=48",40,k);
				sum("ma in (24,25,26,27,28,29,30,37,38,39)",23,k);
			}
		}
		
		private void sum(string exp,int stt,int k)
		{
			DataRow[] r;
			Int16 iRec;
			decimal l_tong=0;
			...
			m.updrec_145(ds.Tables[0],stt,k,l_tong);
		}
	}
}

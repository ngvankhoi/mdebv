...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu07 : System.Windows.Forms.Form
	{
		...
		public frmBieu07(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu07( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu07));
            ...
            this.Load += new System.EventHandler(this.frmBieu07_Load);
		...
		private void frmBieu07_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_07 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten,b.donvi from "+m.user+".bieu_07 a,"+m.user+".dm_07 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_07 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_07 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_07 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_07 where id="+l_id);
					string exp="soluong>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu07(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,Decimal.Parse(r[i]["soluong"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;

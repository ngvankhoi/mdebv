...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu04 : System.Windows.Forms.Form
	{
		...
		public frmBieu04(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu04( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu04));
            ...
            this.Text = "Hoạt động phẫu thuật, thủ thuật";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu04_Load);
		...
		private void frmBieu04_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_04_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_04 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".bieu_04 a,"+m.user+".dm_04 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_04 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".bieu_04 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_04 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_04 where id="+l_id);
					...
					{
						m.upd_bieu04(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c10"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Loại phẫu thuật, thủ thuật";
			TextCol1.Width = 170;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string exp="ma>10",stt="B";
				if (i_row<10 || dataGrid1[i_row,1].ToString()=="A")
				{
					exp="ma>1 and ma<10";
					stt="A";
				}
				...
				{
					if (decimal.Parse(dataGrid1[i_row,i_col].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || (decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString()))>decimal.Parse(dataGrid1[i_row,3].ToString()) || (decimal.Parse(dataGrid1[i_row,7].ToString())+decimal.Parse(dataGrid1[i_row,8].ToString())+decimal.Parse(dataGrid1[i_row,9].ToString()))>decimal.Parse(dataGrid1[i_row,6].ToString()) || (decimal.Parse(dataGrid1[i_row,11].ToString())+decimal.Parse(dataGrid1[i_row,12].ToString()))>decimal.Parse(dataGrid1[i_row,10].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}

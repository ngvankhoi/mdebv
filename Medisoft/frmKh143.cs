...
namespace Medisoft
{
	...
	/// </summary>
	public class frmKh143 : System.Windows.Forms.Form
	{
		...
		public frmKh143(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmKh143( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmKh143));
            ...
            this.butMoi.Location = new System.Drawing.Point(187, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(257, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(327, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(537, 500);
            ...
            this.Text = "Biểu 14.3//Các bệnh dịch lây và các bệnh quan trọng";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmKh143_Load);
		...
		private void frmKh143_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_143_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_143 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_143 a,"+m.user+".kh_dm_143 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_143 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_143 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_143 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_143 where id="+l_id);
					...
					{
						m.upd_kh143(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Tên xã,thành phố huyện trong tỉnh";
			TextCol1.Width = 186;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);

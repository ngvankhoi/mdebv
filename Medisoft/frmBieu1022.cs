...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu1022 : System.Windows.Forms.Form
	{
		...
		public frmBieu1022(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu1022( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu1022));
            ...
            this.Name = "frmBieu1022";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "HĐTC :  Chi tiết về các khoản chi";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu1022_Load);
		...
		private void frmBieu1022_Load(object sender, System.EventArgs e)
		{
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_1022 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from  "+m.user+".bieu_1022 a,"+m.user+".dm_1022 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_1022 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from  "+m.user+".bieu_1022 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from  "+m.user+".bieu_1022 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from  "+m.user+".bieu_1022 where id="+l_id);
					string exp="sotien>0";
					DataRow[] r=ds.Tables[0].Select(exp);
					Int16 iRec=Convert.ToInt16(r.Length);
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_bieu1022(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,Decimal.Parse(r[i]["sotien"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		{
			Color c = Color.Black;
            if (this.dataGrid1[row,1].ToString().Trim()=="B")
			{
				c=Color.Red;
				i_tong=row;
			}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				string exp="",stt="";
				if (i_row>1 || dataGrid1[i_row,1].ToString()=="B")
				{
					exp="ma>2 and ma<>10";
					stt="B";
				}
				if (stt!="")
				{
					...
					l_tong=0;
					foreach(DataRow r1 in ds.Tables[0].Select("ma>2 and ma<9")) l_tong+=decimal.Parse(r1["sotien"].ToString());	
					m.updrec_02(ds.Tables[0],"1",i_col,l_tong);
				}
		...
		{
			string exp="ma>2 and ma<>10",stt="B";
			DataRow[] r;
			Int16 iRec;
			decimal l_tong;
			for(int k=3;k<4;k++)
			{
				...
				l_tong=0;
				foreach(DataRow r1 in ds.Tables[0].Select("ma>2 and ma<9")) l_tong+=decimal.Parse(r1["sotien"].ToString());	
				m.updrec_02(ds.Tables[0],"1",k,l_tong);
			}
		}
	}
}

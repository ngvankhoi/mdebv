...
namespace Medisoft
{
	...
	public class frmKh122 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "Biểu 12.2//Công tác khám chữa bệnh và dịch vụ y tế";
		...
		private void frmKh122_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_122_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_122 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_122 a,"+m.user+".kh_dm_122 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_122 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_122 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_122 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_122 where id="+l_id);
					string exp="c01+c02+c03+c04+c05+c06+c07+c08+c09+c10+c11+c12+c13+c14+c15+c16+c17+c18+c19>0";
					...
					for(Int16 i=0;i<iRec;i++)
					{
						m.upd_kh122(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
							...
							int.Parse(r[i]["c17"].ToString()),int.Parse(r[i]["c18"].ToString()),int.Parse(r[i]["c19"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;

...
namespace Medisoft
{
	...
	public class frmKh121 : System.Windows.Forms.Form
	{
		...
		private void frmKh121_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_121_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_121 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_121 a,"+m.user+".kh_dm_121 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_121 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_121 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_121 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_121 where id="+l_id);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=3 && i_col<=8 && dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,4].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || decimal.Parse(dataGrid1[i_row,6].ToString())>decimal.Parse(dataGrid1[i_row,5].ToString()) || decimal.Parse(dataGrid1[i_row,8].ToString())>decimal.Parse(dataGrid1[i_row,7].ToString()))
					{

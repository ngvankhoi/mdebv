...
namespace Medisoft
{
	...
	public class frmHoantrataisan : System.Windows.Forms.Form
	{
		...
		private void load_grid()
		{
			...
			{
				s=d.losx_sothe(s_mmyy,decimal.Parse(r["sttt"].ToString()));
		...
		private void emp_detail()
		{
			...
			soluong.Text="";stt.Text=d.get_stt(dtct).ToString();d_soluongcu=0;losx.Text="";sothe.Text="";
		}

		private void load_dm()
		{
			dtton=d.get_tutrucct_dutru(s_mmyy,i_makp,s_makho,s_manguon,0,i_nhom);
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			if (!d.upd_hoantrall(s_mmyy,l_id,l_duyet,mabn.Text,0,l_maql,l_idkhoa,s_ngay,int.Parse(lydo.SelectedValue.ToString()),2))
			{
            ...
            foreach (DataRow r1 in dtct.Rows)
            {
                ...
                {
                    d.upd_eve_tables(s_mmyy, itable, i_userid, "upd");
                    d.upd_eve_upd_del(s_mmyy, itable, i_userid, "upd",d.fields(xxx+".d_hoantract","id="+l_id+" and stt="+int.Parse(r1["stt"].ToString())));
                }
                else d.upd_eve_tables(s_mmyy, itable, i_userid, "ins");
                d.upd_hoantract(s_mmyy,l_id, int.Parse(r1["stt"].ToString()),s_ngay, int.Parse(r1["madoituong"].ToString()), int.Parse(r1["makho"].ToString()), int.Parse(r1["mabd"].ToString()), decimal.Parse(r1["soluong"].ToString()), 0, int.Parse(r1["manguon"].ToString()), decimal.Parse(r1["sttt"].ToString()));
            }
			d.updrec_hoantrall(dtll,l_id,mabn.Text,l_maql,l_idkhoa,"",int.Parse(lydo.SelectedValue.ToString()),ghichu.Text,0);
			try
			{
		...
		private void listDmbd_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{
			if (e.KeyCode==Keys.Enter)
			{
				try
				{
					r=d.getrowbyid(dtton,"stt="+decimal.Parse(mabd.Text));
					if (r!=null)
					{
		...
		private void butXoa_Click(object sender, System.EventArgs e)
		{
			...
			d.delrec(dtct,"stt="+decimal.Parse(stt.Text));
			dtct.AcceptChanges();
		...
		private bool upd_table(DataTable dt)
		{
			...
			d.updrec_hoantra_taisan(dt,int.Parse(stt.Text),"",i_mabd,mabd.Text,tenbd.Text,"",dang.Text,makho.Text,d_soluong,5,int.Parse(makho.SelectedValue.ToString()),mahc.Text,int.Parse(manguon.SelectedValue.ToString()),1,(sttt.Text!="")?decimal.Parse(sttt.Text):0,losx.Text,sothe.Text,dtton);
			return true;
		}
		...
		private void soluong_Validated(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (mabd.Text!="" && tenbd.Text!="")
				{
					...
					{
						i_mabd=int.Parse(r["id"].ToString());
						d_soluongton=d.get_slton_nguon(dtton,(sttt.Text!="")?decimal.Parse(sttt.Text):0,int.Parse(makho.SelectedValue.ToString()),i_mabd,int.Parse(manguon.SelectedValue.ToString()),d_soluongcu);

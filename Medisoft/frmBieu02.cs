...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu02 : System.Windows.Forms.Form
	{
		...
		public frmBieu02(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu02( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu02));
            ...
            this.butMoi.Location = new System.Drawing.Point(207, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(277, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(347, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(417, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(557, 500);
            ...
            this.dk.Location = new System.Drawing.Point(463, 4);
            this.dk.Name = "dk";
            this.dk.Size = new System.Drawing.Size(321, 21);
            this.dk.TabIndex = 43;
            // 
            // label2
            // 
            ...
            this.Load += new System.EventHandler(this.frmBieu02_Load);
		...
		private void frmBieu02_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_02_dk").Tables[0];
			load_grid(false);
		...
		private void load_grid(bool dm)
		{
			if (dm)
			{
				ds=m.get_data("select * from "+m.user+".dm_02 where ma<2 order by ma");
				foreach(DataRow r in m.get_data("select * from "+m.user+".btdkp_bv where loai=1 order by makp").Tables[0].Rows)
					m.updrec_bieu(ds.Tables[0],int.Parse(r["makp"].ToString()),r["tenkp"].ToString(),9);
			}
			else load_data();
...
//			if (bPhatsinh)
//			{
//				sql="select a.*,b.stt,b.ten from "+m.user+".bieu_02 a,"+m.user+".dm_02 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
//				ds=m.get_data(sql);
//			}
//			else
//			{
...
//			}
		...
		private void load_value(bool load)
		{
			load_grid(true);
			DataRow[] dr;
			DataTable dt1=m.get_data("select * from "+m.user+".bieu_02 where id="+l_id).Tables[0];
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".bieu_02 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".bieu_02 where id="+l_id);
					...
					{
						m.upd_bieu02(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void AddGridTableStyle()
		{
			...
			TextCol1.HeaderText = "Khám chuyên khoa";
			TextCol1.Width = 198;
			TextCol1.ReadOnly=true;
			ts.GridColumnStyles.Add(TextCol1);
			dataGrid1.TableStyles.Add(ts);
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				{
					decimal tc=decimal.Parse(dataGrid1[i_row,4].ToString())+decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString());
					if (decimal.Parse(dataGrid1[i_row,i_col].ToString())>decimal.Parse(dataGrid1[i_row,3].ToString()) || tc>decimal.Parse(dataGrid1[i_row,3].ToString()))
					{
						MessageBox.Show(lan.Change_language_MessageText("Số liệu không hợp lệ !"),LibMedi.AccessData.Msg);
						dataGrid1[i_row,i_col]=0;
					}

...
namespace Medisoft
{
	...
	public class frmKh01 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.butMoi.Location = new System.Drawing.Point(120, 500);
            ...
            this.butSua.Location = new System.Drawing.Point(209, 500);
            ...
            this.butLuu.Location = new System.Drawing.Point(298, 500);
            ...
            this.butBoqua.Location = new System.Drawing.Point(387, 500);
            ...
            this.butKetthuc.Location = new System.Drawing.Point(565, 500);
            ...
            this.butHuy.Location = new System.Drawing.Point(476, 500);
            ...
            this.Text = "Dân số, đơn vị hành chính, trạm y tế xã";
		...
		private void frmKh01_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_01_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_01 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_01 a,"+m.user+".kh_dm_01 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_01 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_01 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_01 group by id,ngay order by ngay").Tables[0];
		}
		...
		{
			dt1=m.get_data("select * from "+m.user+".kh_bieu_01 order by ngay desc").Tables[0];
			if (dt1.Rows.Count!=0)
			{
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_01 where id="+l_id);
					...
					{
						m.upd_kh01(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=5 && i_col<=8 && dataGrid1[i_row,i_col].ToString()!="")
				{
					decimal tc=decimal.Parse(dataGrid1[i_row,5].ToString())+decimal.Parse(dataGrid1[i_row,6].ToString())+decimal.Parse(dataGrid1[i_row,7].ToString())+decimal.Parse(dataGrid1[i_row,8].ToString());

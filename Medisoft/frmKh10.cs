...
namespace Medisoft
{
	...
	public class frmKh10 : System.Windows.Forms.Form
	{
		...
		private void InitializeComponent()
		{
            ...
            this.Text = "Tình hình chăm sóc sức khoẻ bà mẹ";
		...
		private void frmKh10_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".kh_dm_10_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".kh_dm_10 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from "+m.user+".kh_bieu_10 a,"+m.user+".kh_dm_10 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".kh_dm_10 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from "+m.user+".kh_bieu_10 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from "+m.user+".kh_bieu_10 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (ds.HasChanges())
				{
					m.execute_data("delete from "+m.user+".kh_bieu_10 where id="+l_id);
					...
					{
						m.upd_kh10(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,int.Parse(r[i]["c01"].ToString()),
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (i_col>=23 && i_col<=32 && dataGrid1[i_row,i_col].ToString()!="")
				{
					if (decimal.Parse(dataGrid1[i_row,24].ToString())>decimal.Parse(dataGrid1[i_row,23].ToString()) || decimal.Parse(dataGrid1[i_row,26].ToString())>decimal.Parse(dataGrid1[i_row,25].ToString()) || decimal.Parse(dataGrid1[i_row,28].ToString())>decimal.Parse(dataGrid1[i_row,27].ToString()) || decimal.Parse(dataGrid1[i_row,30].ToString())>decimal.Parse(dataGrid1[i_row,29].ToString()) || decimal.Parse(dataGrid1[i_row,32].ToString())>decimal.Parse(dataGrid1[i_row,31].ToString()))
					{

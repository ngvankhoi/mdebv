...
namespace Medisoft
{
	...
	/// </summary>
	public class frmBieu103 : System.Windows.Forms.Form
	{
		...
		private int i_userid,i_mabv,i_col,i_row=0;
		...
		public frmBieu103(AccessData acc,string hoten,int userid,int mabv)
		{
			//
        ...
        public frmBieu103( string hoten, int userid, int mabv)
        {
            //
            // Required for Windows Form Designer support
            //
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmBieu103));
            ...
            this.Name = "frmBieu103";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "HĐTC : Các khoản tiền không thu được";
            this.WindowState = System.Windows.Forms.FormWindowState.Maximized;
            this.Load += new System.EventHandler(this.frmBieu103_Load);
		...
		private void frmBieu103_Load(object sender, System.EventArgs e)
		{
			...
			load_ngay();
			dt=m.get_data("select * from "+m.user+".dm_103_dk").Tables[0];
			load_grid(false);
		...
		{
			if (dm)	ds=m.get_data("select * from "+m.user+".dm_103 order by ma");
			else load_data();
		...
		private void load_data()
		{
			if (bPhatsinh)
			{
				sql="select a.*,b.stt,b.ten from  "+m.user+".bieu_103 a,"+m.user+".dm_103 b where a.ma=b.ma and a.id="+l_id+" order by a.ma";
				ds=m.get_data(sql);
			}
			else
			{
		...
		{
			ds=m.get_data("select * from "+m.user+".dm_103 order by ma");
			DataRow[] dr;
			foreach(DataRow r in m.get_data("select * from  "+m.user+".bieu_103 where id="+l_id).Tables[0].Rows)
			{
		...
		{
			cmbNgay.DataSource=m.get_data("select ID,to_char(ngay,'dd/mm/yyyy') as NGAY from  "+m.user+".bieu_103 group by id,ngay order by ngay").Tables[0];
		}
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			try
			{
				if (ds.HasChanges())
				{
					m.execute_data("delete from  "+m.user+".bieu_103 where id="+l_id);
					...
					{
						m.upd_bieu103(l_id,int.Parse(r[i]["ma"].ToString()),ngay.Text,Decimal.Parse(r[i]["c01"].ToString()),
							Decimal.Parse(r[i]["c02"].ToString()),Decimal.Parse(r[i]["c03"].ToString()),Decimal.Parse(r[i]["c04"].ToString()),
							Decimal.Parse(r[i]["c05"].ToString()),Decimal.Parse(r[i]["c06"].ToString()),i_userid);
					}
				}
				ena_object(false);
				decimal id=l_id;
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			try
			{
				...
				if (r1!=null) ten.Text=r1[1].ToString();
				else ten.Text="";

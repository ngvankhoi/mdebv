...
namespace dllDanhmucMedisoft
{
	...
	public class frmChonvp : System.Windows.Forms.Form
	{
		...
		private void frmChonvp_Load(object sender, System.EventArgs e)
		{
            ...
            //{
            //    updrec(s_mucvp.Substring(j, 7));
            //    j += 8;
            //}
            string[] arrMucvp = s_mucvp.Trim().Trim(',').Split(',');
            for (int j = 0; j < arrMucvp.Length; j++)
            {
                updrec(arrMucvp[j]);
            }
			...
			if (loai.Items.Count>0)	load_mavp(decimal.Parse(loai.SelectedValue.ToString()));
		}

		private void load_mavp(decimal id)
		{
		...
		private void loai_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			...
			{
				get_mavp();
				load_mavp(decimal.Parse(loai.SelectedValue.ToString()));
			}
		}
		...
		private void butOk_Click(object sender, System.EventArgs e)
		{
            ...
            for (int i = 0; i < loai.Items.Count; i++)
            {
                ...
                {
                    s_loaivp += dtloai.Rows[i]["id"].ToString().Trim().PadLeft(3, '0') + ",";
                }
            }
			m.close();this.Close();
		}

		private void updrec(string ma)
		{
		...
		private void get_mavp()
		{
			...
			{
                if (mavp.GetItemChecked(i)) updrec(dtmavp.Rows[i]["id"].ToString());
                else m.delrec(dt, "ma='" + dtmavp.Rows[i]["id"].ToString() + "'");
			}
			if (dt.Rows.Count>0) s_mucvp="";
			foreach(DataRow r in dt.Rows) s_mucvp+=r["ma"].ToString().Trim()+",";
		}

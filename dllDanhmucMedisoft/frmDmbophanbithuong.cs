...
namespace dllDanhmucMedisoft
{
	...
	/// </summary>
	public class frmDmbophanbithuong : System.Windows.Forms.Form
	{
		...
		public frmDmbophanbithuong(AccessData acc,int userid)
		{
			//
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDmbophanbithuong));
            ...
            this.Name = "frmDmbophanbithuong";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Danh mục bộ phận bị thương";
            this.KeyDown += new System.Windows.Forms.KeyEventHandler(this.frmDmbophanbithuong_KeyDown);
            this.Load += new System.EventHandler(this.frmDmbophanbithuong_Load);
		...
		private void frmDmbophanbithuong_Load(object sender, System.EventArgs e)
		{
            user = m.user; itable = m.tableid("", "dmbophan");
			load_grid();
			AddGridTableStyle();
		...
		{
            dt = m.get_data("select ma,ten,stt,nvl(viettat,'') as viettat from " + user + ".dmbophan order by ma").Tables[0];
		...
		private void AddGridTableStyle()
		{
			...
			TextCol.HeaderText = "Bộ phận bị thương";
			TextCol.Width = 200;
			ts.GridColumnStyles.Add(TextCol);
            dataGrid1.TableStyles.Add(ts);
		...
		{
			if (!kiemtra()) return ;
            if (m.get_data("select * from " + user + ".dmbophan where ma='" + ma.Text + "'").Tables[0].Rows.Count != 0)
            {
            ...
            if (!m.upd_dmdiadiem("dmbophan", ma.Text, ten.Text, txtstt.Text, txtviettat.Text))
			{
				MessageBox.Show(lan.Change_language_MessageText("Không cập nhật được thông tin địa điểm !"));
				return;
			}
            m.upd_dmdiadiem("dmbophan", ma.Text, ten.Text, txtstt.Text, txtviettat.Text);
			load_grid();
			ena_object(false);
		}
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show(lan.Change_language_MessageText("Đồng ý hủy thông tin này !"),LibMedi.AccessData.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                m.upd_eve_upd_del(itable, i_userid, "del", m.fields(user + ".dmbophan", "ma='" + ma.Text + "'"));
                m.execute_data("delete from " + user + ".dmbophan where ma='" + ma.Text + "'");
				load_grid();
			}
		}
		...
		private void frmDmbophanbithuong_KeyDown(object sender, System.Windows.Forms.KeyEventArgs e)
		{

...
namespace dllDanhmucMedisoft
{
	...
	/// </summary>
	public class frmDmxutri : System.Windows.Forms.Form
	{
		...
		public frmDmxutri(AccessData acc,int userid)
		{
			//
		...
		{
            System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(frmDmxutri));
            ...
            this.Name = "frmDmxutri";
            this.StartPosition = System.Windows.Forms.FormStartPosition.CenterScreen;
            this.Text = "Danh mục xử trí sau tai nạn";
		...
		{
            user = m.user; itable = m.tableid("", "dmxutri");
			load_grid();
			AddGridTableStyle();
		...
		{
            dt = m.get_data("select ma,ten,stt,nvl(viettat,'') as viettat from " + user + ".dmxutri order by ma").Tables[0];
		...
		{
			if (!kiemtra()) return ;
            if (m.get_data("select * from " + user + ".dmxutri where ma='" + ma.Text + "'").Tables[0].Rows.Count != 0)
            {
            ...
            if (!m.upd_dmdiadiem("dmxutri", ma.Text, ten.Text, txtstt.Text, txtviettat.Text))
			{
				MessageBox.Show(lan.Change_language_MessageText("Không cập nhật được thông tin địa điểm !"));
				return;
			}
            m.upd_dmdiadiem("dmxutri", ma.Text, ten.Text, txtstt.Text, txtviettat.Text);
			load_grid();
			ena_object(false);
		}
		...
		private void butHuy_Click(object sender, System.EventArgs e)
		{
			...
			if (MessageBox.Show(lan.Change_language_MessageText("Đồng ý hủy thông tin này !"),LibMedi.AccessData.Msg,MessageBoxButtons.YesNo,MessageBoxIcon.Question)==DialogResult.Yes)
			{
                ...
                m.upd_eve_upd_del(itable, i_userid, "del", m.fields(user + ".dmxutri", "ma='" + ma.Text + "'"));
                m.execute_data("delete from " + user + ".dmxutri where ma='" + ma.Text + "'");
				load_grid();
			}
		}

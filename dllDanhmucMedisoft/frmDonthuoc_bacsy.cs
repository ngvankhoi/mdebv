	...
	public class frmDonthuoc_bacsy : System.Windows.Forms.Form
	{
		...
		private void load_maicd()
		{
			...
			if (lmabs.Items.Count>0)
			{
				...
				l_id=(lmaicd.Items.Count>0)?decimal.Parse(lmaicd.SelectedValue.ToString()):0;
			}
		}
		...
		private void load_grid()
		{
			...
			{
				sql="select id,ma,trim(ten)||' '||hamluong as ten,dang,nullif(tenhc,' ') as tenhc from ";
		...
		private void load_chitiet()
		{
			if (bDanhmuc_nhathuoc)
			{
				sql="select b.stt,b.mabd,a.ma,trim(a.ten)||' '||a.hamluong as ten,a.dang,nullif(a.tenhc,' ') as tenhc,b.soluong,b.cachdung ";
		...
		private void lmaicd_SelectedIndexChanged(object sender, System.EventArgs e)
		{
			if (this.ActiveControl==lmaicd)
			{
				try
				{
					l_id=decimal.Parse(lmaicd.SelectedValue.ToString());
				}
				catch{l_id=0;}
				load_chitiet();
			}
		...
		private void butThem_Click(object sender, System.EventArgs e)
		{
			if (dsdm.Tables[0].Select("chon=true").Length>0)
			{
				foreach(DataRow r in dsdm.Tables[0].Select("chon=true"))
                    m.updrec_donthuoc(dt, int.Parse(r["id"].ToString()), r["ma"].ToString(), r["ten"].ToString(), r["dang"].ToString(), r["tenhc"].ToString(), d.get_stt(dt), "Khác", 0);
				foreach (DataRow row in dsdm.Tables[0].Rows) row["chon"]=false;
			}
		}

...
namespace dllDanhmucMedisoft
{
	...
	public class frmKbpttt : System.Windows.Forms.Form
	{
        ...
        private TextBox tim;
        private CheckBox chktsu;
        private CheckBox chkSande;
		/// <summary>
		...
		{
            sql = "select mapt,mapttt,noi_dung,coalesce(dacbiet,' ') as dacbiet,coalesce(loai1,' ') as loai1,coalesce(loai2,' ') as loai2,coalesce(loai3,' ') as loai3,loaipt,id_pttt,id_muc,mavp,tiensu,sande from " + user + ".dmpttt where 1=1 ";
		...
		private void butLuu_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				if (r!=null && r1!=null)
				{
					...
					m.upd_dmpttt(muc.SelectedValue.ToString()+ma.Text,mapttt,ten.Text,r1["db"].ToString(),r1["l1"].ToString(),r1["l2"].ToString(),r1["l3"].ToString(),int.Parse(loaipt.SelectedValue.ToString()),int.Parse(r["id_pttt"].ToString()),int.Parse(r["id_muc"].ToString()),(mavp.SelectedIndex!=-1)?int.Parse(mavp.SelectedValue.ToString()):0);
                    m.execute_data("update " + user + ".dmpttt set tiensu=" + (chktsu.Checked ? "1" : "0") + ",sande=" + (chkSande.Checked ? "1" : "0") + " where mapt='" + muc.SelectedValue.ToString() + ma.Text+"'"); //Khuong 11/11/2011 them sande
				}
				ena_object(false);
				load_grid();
			}
		...
		private void dataGrid1_CurrentCellChanged(object sender, System.EventArgs e)
		{
			try
			{
                ...
                //8,9
                chktsu.Checked = dataGrid1[i, 8].ToString()=="1";
                chkSande.Checked = dataGrid1[i, 9].ToString() == "1";
